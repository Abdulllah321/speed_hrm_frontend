...
import { NetSalesSummaryTableRow, NetSalesSummaryTotals } from "./types";
import { ChevronRight, ChevronDown, UnfoldVertical, FoldVertical, Info, Layers } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Tooltip, TooltipTrigger, TooltipContent } from "@/components/ui/tooltip";

interface NetSalesSummaryTableProps {
  rows: NetSalesSummaryTableRow[];
...
export function NetSalesSummaryTable({
  rows,
  grandTotals,
  onToggleNode,
  onExpandAll,
  onCollapseAll,
}: NetSalesSummaryTableProps) {
    ...
    <div className="space-y-2.5">
      ...
      <div className="flex items-center justify-between px-1 no-print">
        <span className="text-xs font-semibold text-slate-600 dark:text-slate-400 flex items-center gap-1.5">
          Showing <span className="font-bold text-slate-900 dark:text-slate-100">{rows.length.toLocaleString()}</span> net sales summary matrix rows
        </span>
      ...
      <div className="border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xs bg-white dark:bg-slate-900 overflow-hidden no-print">
        <div ref={parentRef} className="overflow-auto max-h-[700px] relative">
          <table className="w-full text-left border-collapse min-w-[1900px] text-xs">
            ...
            <thead className="sticky top-0 z-20 bg-slate-100 dark:bg-slate-800/90 text-slate-700 dark:text-slate-300 uppercase text-[10px] font-mono tracking-wider border-b border-slate-200 dark:border-slate-700 shadow-2xs backdrop-blur-xs">
              <tr>
                ...
                <th className="py-3 px-3 w-[140px] shrink-0 border-r border-slate-200 dark:border-slate-700">Brand</th>
                <th className="py-3 px-3 w-[130px] shrink-0 border-r border-slate-200 dark:border-slate-700">SKU / Barcode</th>
                ...
                {/* Return Qty */}
                <th className="py-3 px-3 w-[80px] shrink-0 border-r border-slate-200 dark:border-slate-700 text-right text-rose-600 dark:text-rose-400 font-bold">
                  Ret Qty
                </th>

                {/* Net Qty */}
                <th className="py-3 px-3 w-[90px] shrink-0 border-r border-slate-200 dark:border-slate-700 text-right font-bold text-slate-900 dark:text-slate-100">
                  Net Qty
                </th>
                ...
                {/* Return Amount */}
                <th className="py-3 px-3 w-[120px] shrink-0 border-r border-slate-200 dark:border-slate-700 text-right text-rose-600 dark:text-rose-400 font-bold">
                  Returns
                </th>
                  ...
                  <div className="flex items-center justify-end gap-1">
                    ...
                    <Tooltip>
                      ...
                      <TooltipContent side="top" className="max-w-xs text-[11px] font-medium bg-slate-900 text-slate-100">
                        Net sales revenue collected after returns & discounts.
                      </TooltipContent>
                    </Tooltip>
                  </div>
                </th>
            ...
            <tbody>
              ...
              {rows.length === 0 ? (
                <tr>
                  <td colSpan={13} className="p-14 text-center text-muted-foreground font-medium text-xs">
                    No net sales summary records found matching the selected store, cashier, or date range filters.
                  </td>
                </tr>
              ) : (
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-2.5 px-3 border-r border-slate-100 dark:border-slate-800/60 text-right font-mono font-semibold">
                        {isItem ? item.soldQty : t.totalItemsSold.toLocaleString()}
                      </td>

                      {/* Return Qty */}
                      <td className="py-2.5 px-3 border-r border-slate-100 dark:border-slate-800/60 text-right font-mono font-semibold text-rose-600 dark:text-rose-400">
                        {isItem ? item.returnQty : t.totalItemsReturned.toLocaleString()}
                      </td>

                      {/* Net Qty */}
                      <td className="py-2.5 px-3 border-r border-slate-100 dark:border-slate-800/60 text-right font-mono font-bold text-slate-900 dark:text-slate-100">
                        {isItem ? item.netQty : t.netItems.toLocaleString()}
                      </td>
                      ...
                      <td className="py-2.5 px-3 border-r border-slate-100 dark:border-slate-800/60 text-right font-mono text-slate-700 dark:text-slate-300">
                        {formatVal(t.grossSalesAmount)}
                      </td>

                      {/* Return Amount */}
                      <td className="py-2.5 px-3 border-r border-slate-100 dark:border-slate-800/60 text-right font-mono font-semibold text-rose-600 dark:text-rose-400">
                        {formatVal(t.returnAmount)}
                      </td>
            ...
            <tfoot className="sticky bottom-0 z-20 bg-slate-100 dark:bg-slate-800 text-slate-900 dark:text-slate-100 uppercase text-[11px] font-mono font-bold shadow-sm border-t-2 border-slate-300 dark:border-slate-700">
              <tr>
                ...
                <td className="py-3 px-3 border-r border-slate-200 dark:border-slate-700 text-right font-mono text-rose-600 dark:text-rose-400">
                  {grandTotals.totalItemsReturned.toLocaleString()}
                </td>
                <td className="py-3 px-3 border-r border-slate-200 dark:border-slate-700 text-right font-mono text-slate-900 dark:text-slate-100">
                  {grandTotals.netItems.toLocaleString()}
                </td>
                <td className="py-3 px-3 border-r border-slate-200 dark:border-slate-700 text-right font-mono">
                  {formatVal(grandTotals.grossSalesAmount)}
                </td>
                <td className="py-3 px-3 border-r border-slate-200 dark:border-slate-700 text-right font-mono text-rose-600 dark:text-rose-400">
                  {formatVal(grandTotals.returnAmount)}
                </td>
                ...
                <td className="py-3 px-3.5 text-right font-mono text-emerald-600 dark:text-emerald-400">
                  {formatVal(grandTotals.netSalesAmount)}
                </td>
              </tr>
            </tfoot>
          </table>

...
export function useSalesRegisterData(reportData: SalesRegisterReportData | null) {
  ...
  const grandTotals = useMemo<SalesRegisterTotals>(() => {
  ...
  const flatRows = useMemo<SalesRegisterTableRow[]>(() => {
    const rows: SalesRegisterTableRow[] = [];
    if (!reportData) return rows;

    const q = searchQuery.toLowerCase().trim();

    const filterInvoice = (inv: SalesRegisterInvoiceNode) => {
    ...
    const flattenInvoices = (invoicesList: SalesRegisterInvoiceNode[], depthOffset: number, prefix: string) => {

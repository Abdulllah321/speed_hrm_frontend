...
import { SalesRegisterInvoiceNode, SalesRegisterTotals } from "./types";

export async function generateSalesRegisterPdf(opts: {
  ...
  const html = `
    ...
    <html>
      <head>
        <title>Sales Register Report - ${dateStr}</title>
        <style>
      ...
      <body>
        <div class="header">
          <div>
            <div class="title">POS Sales Register Report</div>
        ...
        <table>
          ...
          <tfoot>
            <tr>
              <td colspan="6">GRAND TOTAL (ALL SELECTED REGISTER INVOICES)</td>

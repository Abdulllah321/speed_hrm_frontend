...
import { WholesaleInvoiceRegisterData } from "./types";
import { WholesaleInvoiceHeader } from "./wholesale-invoice-header";
import { WholesaleInvoiceFilters } from "./wholesale-invoice-filters";
import { WholesaleInvoiceTable } from "./wholesale-invoice-table";
import { useWholesaleInvoiceData } from "./use-wholesale-invoice-data";
import { useReportSse } from "@/hooks/use-report-sse";
import {
  queueWholesaleInvoiceRegisterPreview,
  getWholesaleInvoiceRegisterResult,
  queueWholesaleInvoiceRegisterExport,
  getWholesaleInvoiceRegisterExportStatus,
} from "@/lib/actions/wholesale-invoice-register";
...
export function WholesaleInvoiceView() {
  ...
  const sseState = useReportSse(previewJobId, "wholesale-invoice-register");
    ...
    (targetPreset?: string, targetRange?: DateRange) => {
      ...
      startTransition(async () => {
        try {
          ...
          const res = await queueWholesaleInvoiceRegisterPreview({
            customerId,
            startDate,
            endDate,
          });
  ...
  useEffect(() => {
    if (
      ...
      getWholesaleInvoiceRegisterResult(previewJobId)
        .then((res) => {
          if (res && res.status && res.data) {
            setReportData(res.data);
            toast.success("Wholesale invoice register updated");
          } else {
            toast.error("Failed to load completed wholesale invoice register dataset");
          }
        })
        .catch(() => {
          toast.error("Error retrieving completed wholesale invoice preview");
        })
        .finally(() => {
  ...
  const {
    ...
    grandTotals,
    treeData,
  } = useWholesaleInvoiceData(reportData, {
    reportType,
    selectedCustomerIds,
  ...
  const triggerExportJob = async (format: "xlsx" | "pdf") => {
    try {
      ...
      const customerId = selectedCustomerIds.length === 1 ? selectedCustomerIds[0] : undefined;
      const res = await queueWholesaleInvoiceRegisterExport({
        customerId,
      ...
      const pollInterval = setInterval(async () => {
        const statusRes = await getWholesaleInvoiceRegisterExportStatus(jobId);
        if (statusRes.status && statusRes.data) {
          if (statusRes.data.state === "completed") {
            ...
            const fileName = `wholesale-invoice-register-${dateStr}.${format}`;
            const downloadUrl = `${apiOrigin}/api/sales/reports/wholesale-invoice-register/export/${jobId}/download/${encodeURIComponent(fileName)}`;
  ...
  return (
    <div className="p-6 space-y-6 max-w-[1750px] mx-auto relative">
      <WholesaleInvoiceHeader totals={grandTotals} />

      <WholesaleInvoiceFilters

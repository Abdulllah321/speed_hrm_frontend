...
import {
  WholesaleInvoiceRegisterData,
  WholesaleInvoiceFlatRecord,
  GroupingLevels,
  WholesaleInvoiceTreeNode,
  WholesaleInvoiceTotals,
} from "./types";
import { DateRange } from "react-day-picker";

interface UseWholesaleInvoiceDataProps {
...
const emptyTotals = (): WholesaleInvoiceTotals => ({
  totalItems: 0,
  grossAmount: 0,
  wostAmount: 0,
  discountAmount: 0,
...
export function useWholesaleInvoiceData(
  initialData: WholesaleInvoiceRegisterData | null,
  options: UseWholesaleInvoiceDataProps
) {
  ...
  const { grandTotals, treeData } = useMemo(() => {
    ...
    if (groupingLevels.invoice || groupingLevels.document) levels.push("invoice");
    ...
    if (levels.length === 0) {
      levels.push(isSeparate ? "customer" : "invoice");
    }

    const tree: WholesaleInvoiceTreeNode[] = [];
    ...
    filteredFlatItems.forEach((item) => {
      ...
      const itemTotals: WholesaleInvoiceTotals = {
      ...
      for (let i = 0; i < levels.length; i++) {
        ...
        let nodeVal = "";
        let extraFields: Partial<WholesaleInvoiceTreeNode> = {};
        ...
        if (levelName === "customer") {
          ...
          if (item.invoiceDate) {
            nodeVal = new Date(item.invoiceDate).toLocaleDateString();
          } else {
            nodeVal = "Unknown Date";
          }
        } else if (levelName === "invoice" || levelName === "document") {
          nodeVal = item.invoiceNo || "Unknown Invoice";
          extraFields.invoiceNo = item.invoiceNo;
          extraFields.invoiceDate = item.invoiceDate;
        } else if (levelName === "salesPerson") {
          nodeVal = "N/A"; // Wholesale Invoice doesn't typically have a salesPerson directly
        ...
        if (!existingNode) {
          existingNode = {
            level: levelName === "document" ? "invoice" : levelName,
            value: nodeVal,
            totals: emptyTotals(),
            ...extraFields,
            children: [],
          ...
          if (!(currentLevelNodes as any)._childMap) {
            (currentLevelNodes as any)._childMap = new Map<string, WholesaleInvoiceTreeNode>();
          }

...
import { GrossSalesSummaryCategoryNode, GrossSalesSummaryTotals } from "./types";
...
export async function generateGrossSalesSummaryPdf(opts: {
  ...
  const rowsHtml = categories
    ...
    <tr>
      ...
      <td>${cat.brandName}</td>
      <td style="text-align: right;">${cat.totals.totalItems}</td>
      <td style="text-align: right;">Rs. ${cat.totals.grossAmount.toLocaleString(undefined, { minimumFractionDigits: 2 })}</td>
      ...
      <td style="text-align: right; font-weight: bold; color: #047857;">Rs. ${cat.totals.netAmount.toLocaleString(undefined, { minimumFractionDigits: 2 })}</td>
    </tr>
  `,
    )
    .join("");
  ...
  const html = `
    ...
    <html>
      ...
      <body>
        <div class="header">
          <div>
            <div class="title">POS Gross Sales Category Summary</div>
        ...
        <table>
          ...
          <tfoot>
            <tr>
              <td colspan="2">GRAND TOTAL (ALL PRODUCT CATEGORIES)</td>

...
import { GrossSalesSummaryReportData } from "./types";
...
export function GrossSalesSummaryView() {
  ...
  const [reportData, setReportData] = useState<GrossSalesSummaryReportData | null>(null);
    ...
    startTransition(async () => {
      try {
        ...
        if (res && res.status && res.data?.jobId) {
          ...
          toast.error(res?.message || "Failed to queue gross sales summary calculation");
        }
      } catch (err: any) {
        toast.error("Error queueing gross sales summary calculation job");
      } finally {

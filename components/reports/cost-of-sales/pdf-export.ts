  ...
  for (const brand of brands) {
    rowsHtml += `
      <tr class="level-brand">
        <td colspan="4" style="padding-left: 8px;">BRAND: ${brand.brandName.toUpperCase()}</td>
    ...
    for (const div of brand.divisions) {
      rowsHtml += `
        <tr class="level-division">
          <td colspan="4" style="padding-left: 18px;">DIVISION: ${div.divisionName.toUpperCase()}</td>
        ...
        for (const cat of gender.categories) {
          rowsHtml += `
            <tr class="level-category">
              <td colspan="4" style="padding-left: 28px;">CATEGORY: ${cat.categoryName.toUpperCase()}</td>
          ...
          for (const prod of cat.products) {
            rowsHtml += `
              <tr class="level-article">
                <td style="padding-left: 38px; font-weight: bold; color: #0369a1;">${prod.description}</td>
            ...
            for (const item of prod.sizes) {
              rowsHtml += `
                <tr class="level-variant">
                  <td style="padding-left: 48px; color: #64748b; font-style: italic;">— Variant Item</td>
  ...
  return `
    ...
    <head>
      ...
      <style>
        ...
        .title { font-size: 12px; font-weight: bold; color: #0369a1; text-transform: uppercase; margin-top: 2px; }
        ...
        .level-brand { background-color: #1e293b; color: #ffffff; font-weight: bold; }
        .level-division { background-color: #334155; color: #ffffff; font-weight: bold; }
        .level-category { background-color: #475569; color: #ffffff; font-weight: bold; }
      ...
      <table>
        <thead>
          <tr>
            ...
            <th style="width: 8%; text-align: right;">Sold Qty</th>
            <th style="width: 9%; text-align: right;">Unit Cost</th>
            <th style="width: 10%; text-align: right;">Total Cost (COGS)</th>
            <th style="width: 10%; text-align: right;">Total Revenue</th>
            <th style="width: 9%; text-align: right;">Gross Profit</th>
            <th style="width: 6%; text-align: right;">Margin %</th>
          </tr>
        </thead>
        <tbody>
          ${rowsHtml}
        ...
        <tfoot>
          <tr class="grand-totals">
            <td colspan="4">GRAND TOTALS (ALL OUTLETS & WAREHOUSES)</td>

...
export function AllianceBulkUploadModal({
    open,
    onOpenChange,
    onSuccess,
    uploadId,
    onUploadIdChange,
}: AllianceBulkUploadModalProps) {
    ...
    const { data, speed, isComplete, isValidated, isValidating, isFailed, isProcessing, isCancelled } =
        useUploadProgress(activeId, 'alliance');
    ...
    const handleUpload = async () => {
        ...
        try {
            const response = await fetch(`${getApiBaseUrl()}/pos-config/alliances/bulk-upload`, {
                method: 'POST',
                body: formData,
                credentials: 'include',
            });
            ...
            if (result.status && result.data?.uploadId) {
                ...
                toast.success('File uploaded. Alliance validation started...');
            } else {
                toast.error(result.message || 'Failed to initiate Alliance upload');
            }
        } catch (error) {
    ...
    const handleConfirm = async () => {
        ...
        try {
            const response = await fetch(
                `${getApiBaseUrl()}/pos-config/alliances/bulk-upload/${activeId}/confirm`,
                { method: 'POST', credentials: 'include' },
            );
            const result = await response.json();
            if (result.status) {
                toast.success('Alliance import started');
            } else {
                toast.error(result.message || 'Failed to start Alliance import');
    ...
    const handleCancel = async () => {
        ...
        try {
            const response = await fetch(
                `${getApiBaseUrl()}/pos-config/alliances/bulk-upload/${activeId}`,
                { method: 'DELETE', credentials: 'include' },
            );
            const result = await response.json();
            if (result.status) {
                toast.info('Alliance job cancelled');
                onUploadIdChange?.(null);
                setInternalUploadId(null);
            }
        } catch (error) {
    ...
    const downloadTemplate = () => {
        window.open(`${getApiBaseUrl()}/pos-config/alliances/bulk-upload/template/download`, '_blank');
    };

    const downloadErrorReport = () => {
        if (!activeId) return;
        window.open(
            `${getApiBaseUrl()}/pos-config/alliances/bulk-upload/${activeId}/error-report`,
            '_blank',
        );
    };
            ...
            >
                <DialogHeader className="p-6 pb-2 border-b bg-muted/30 shrink-0">
                    ...
                    <DialogDescription className="text-sm">
                        Upload your bank alliance discount sheet. Each row represents one BIN number — rows sharing the same alliance name are grouped automatically.
                    </DialogDescription>
                </DialogHeader>
                            ...
                            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                                    ...
                                    {file ? (
                                        ...
                                        <>
                                            ...
                                            <div className="text-center space-y-2">
                                                <p className="font-bold text-xl">Upload Alliance Discount Sheet</p>
                                                <p className="text-sm text-muted-foreground max-w-[340px]">
                                                    Drag and drop your CSV or Excel file with bank alliance discounts, or click to browse.
                                                </p>
                                            </div>
                                ...
                                <div className="flex items-center justify-between p-4 bg-primary/5 rounded-xl border border-primary/10 shadow-sm transition-all hover:shadow-md">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <p className="font-bold text-sm">Download Alliance Template</p>
                                            <p className="text-xs text-muted-foreground">
                                                Columns: S.No, Account Code, BANK, Alliance Name, Expiry, BIN Numbers, Card Name, Card Type, Discount Capping
                                            </p>
                                        </div>
                                    </div>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="flex gap-4 p-4 rounded-xl border bg-card/50 transition-colors hover:bg-card">
                                        ...
                                        <div className="space-y-1">
                                            <p className="text-sm font-bold">Smart BIN Grouping</p>
                                            <p className="text-xs text-muted-foreground leading-relaxed">
                                                Multiple rows with the same alliance name are merged into one record with all BINs.
                                            </p>
                                        </div>
                                    </div>
                                    <div className="flex gap-4 p-4 rounded-xl border bg-card/50 transition-colors hover:bg-card">
                                        ...
                                        <div className="space-y-1">
                                            <p className="text-sm font-bold">Upsert Safe</p>
                                            <p className="text-xs text-muted-foreground leading-relaxed">
                                                Existing alliances are updated with new BINs — no duplicates created.
                                            </p>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div className="space-y-8 animate-in fade-in duration-500">
                                        ...
                                        <div className="space-y-2">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <p className="text-sm font-bold text-primary uppercase tracking-widest">
                                                    {isValidating
                                                        ? 'Phase 1: Validating Alliances'
                                                        : data?.status === 'processing'
                                                        ? 'Phase 2: Importing Alliances'
                                                        : 'Status'}
                                                </p>
                                            </div>
                                ...
                                {isValidated && (
                                    <div className="p-6 rounded-2xl border-2 border-dashed bg-card space-y-4 animate-in zoom-in-95 duration-500">
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                <h4 className="font-black text-lg">Alliance Validation Complete</h4>
                                ...
                                {data?.status === 'completed' && (
                                    <div className="p-8 bg-green-500/5 border-2 border-green-500/20 rounded-3xl flex flex-col items-center gap-4 text-center animate-in zoom-in-95 duration-500">
                                        ...
                                        <div className="space-y-1">
                                            <h3 className="text-2xl font-black text-green-700">Alliance Import Successful!</h3>
                                            <p className="text-green-600/80 font-medium">
                                                {data?.successRecords} alliance rows have been added to your system.
                                            </p>
                                        </div>
                                    </div>
                                )}
                        ...
                        <div className="max-w-[300px]">
                            {isProcessing && (
                                <div className="flex items-center gap-2 text-xs text-muted-foreground font-semibold italic">
                                    ...
                                    Processing alliances in the background...
                                </div>
                            )}
                        </div>

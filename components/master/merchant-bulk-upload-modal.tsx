...
export function MerchantBulkUploadModal({
    open,
    onOpenChange,
    onSuccess,
    uploadId,
    onUploadIdChange,
}: MerchantBulkUploadModalProps) {
    ...
    const { data, speed, isComplete, isValidated, isValidating, isFailed, isProcessing, isCancelled } =
        useUploadProgress(activeId, 'merchant');
    ...
    const handleUpload = async () => {
        ...
        try {
            const response = await fetch(`${getApiBaseUrl()}/pos-config/merchants/bulk-upload`, {
                method: 'POST',
                body: formData,
                credentials: 'include',
            });
            ...
            if (result.status && result.data?.uploadId) {
                ...
                toast.success('File uploaded. Merchant validation started...');
            } else {
                toast.error(result.message || 'Failed to initiate Merchant upload');
            }
        } catch (error) {
    ...
    const handleConfirm = async () => {
        ...
        try {
            const response = await fetch(
                `${getApiBaseUrl()}/pos-config/merchants/bulk-upload/${activeId}/confirm`,
                { method: 'POST', credentials: 'include' },
            );
            const result = await response.json();
            if (result.status) {
                toast.success('Merchant import started');
            } else {
                toast.error(result.message || 'Failed to start Merchant import');
    ...
    const handleCancel = async () => {
        ...
        try {
            const response = await fetch(
                `${getApiBaseUrl()}/pos-config/merchants/bulk-upload/${activeId}`,
                { method: 'DELETE', credentials: 'include' },
            );
            const result = await response.json();
            if (result.status) {
                toast.info('Merchant import job cancelled');
                onUploadIdChange?.(null);
                setInternalUploadId(null);
            }
        } catch (error) {
    ...
    const downloadTemplate = () => {
        window.open(`${getApiBaseUrl()}/pos-config/merchants/bulk-upload/template/download`, '_blank');
    };

    const downloadErrorReport = () => {
        if (!activeId) return;
        window.open(
            `${getApiBaseUrl()}/pos-config/merchants/bulk-upload/${activeId}/error-report`,
            '_blank',
        );
    };
            ...
            >
                <DialogHeader className="p-6 pb-2 border-b bg-muted/30 shrink-0">
                    ...
                    <DialogDescription className="text-sm">
                        Upload your POS merchant configuration sheet. Each row maps a location by its Tag ID (Location Code) and configures the bank, merchant code, commission rate, and Bank GL code.
                    </DialogDescription>
                </DialogHeader>
                            ...
                            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                                    ...
                                    {file ? (
                                        ...
                                        <>
                                            ...
                                            <div className="text-center space-y-2">
                                                <p className="font-bold text-xl">Upload Merchant Configuration Sheet</p>
                                                <p className="text-sm text-muted-foreground max-w-[340px]">
                                                    Drag and drop your CSV or Excel file with POS merchant configurations, or click to browse.
                                                </p>
                                            </div>
                                ...
                                <div className="flex items-center justify-between p-4 bg-primary/5 rounded-xl border border-primary/10 shadow-sm transition-all hover:shadow-md">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <p className="font-bold text-sm">Download Merchants Template</p>
                                            <p className="text-xs text-muted-foreground">
                                                Required Columns: CostCentre, Tag ID, Description, Bank, Merchant code, Commission Rate %, Bank GL Code
                                            </p>
                                        </div>
                                    </div>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="flex gap-4 p-4 rounded-xl border bg-card/50 transition-colors hover:bg-card">
                                        ...
                                        <div className="space-y-1">
                                            <p className="text-sm font-bold">Natural Key Upserts</p>
                                            <p className="text-xs text-muted-foreground leading-relaxed">
                                                Combination of Tag ID + Merchant code acts as natural key. Existing merchants are updated without creating duplicates.
                                            </p>
                                        </div>
                                    </div>
                                    <div className="flex gap-4 p-4 rounded-xl border bg-card/50 transition-colors hover:bg-card">
                                        ...
                                        <div className="space-y-1">
                                            <p className="text-sm font-bold">Comprehensive Validation</p>
                                            <p className="text-xs text-muted-foreground leading-relaxed">
                                                Instantly validates active GL codes, locations by Tag ID, and commission rates in real-time.
                                            </p>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div className="space-y-8 animate-in fade-in duration-500">
                                        ...
                                        <div className="space-y-2">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <p className="text-sm font-bold text-primary uppercase tracking-widest">
                                                    {isValidating
                                                        ? 'Phase 1: Validating Merchants'
                                                        : data?.status === 'processing'
                                                        ? 'Phase 2: Importing Merchants'
                                                        : 'Status'}
                                                </p>
                                            </div>
                                ...
                                {isValidated && (
                                    <div className="p-6 rounded-2xl border-2 border-dashed bg-card space-y-4 animate-in zoom-in-95 duration-500">
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                <h4 className="font-black text-lg">Merchant Validation Complete</h4>
                                ...
                                {data?.status === 'completed' && (
                                    <div className="p-8 bg-green-500/5 border-2 border-green-500/20 rounded-3xl flex flex-col items-center gap-4 text-center animate-in zoom-in-95 duration-500">
                                        ...
                                        <div className="space-y-1">
                                            <h3 className="text-2xl font-black text-green-700">Merchant Import Successful!</h3>
                                            <p className="text-green-600/80 font-medium">
                                                {data?.successRecords} merchant configuration rows have been imported to your system.
                                            </p>
                                        </div>
                                    </div>
                                )}
                        ...
                        <div className="max-w-[300px]">
                            {isProcessing && (
                                <div className="flex items-center gap-2 text-xs text-muted-foreground font-semibold italic">
                                    ...
                                    Processing merchants in the background...
                                </div>
                            )}
                        </div>

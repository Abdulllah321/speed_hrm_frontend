...
export function GenericBulkUploadModal({
    open,
    onOpenChange,
    onSuccess,
    uploadId,
    ...
    description,
    icon: Icon,
    templateFileName = `${uploadType}-upload-template.csv`,
    ...
    const { data, speed, isComplete, isValidated, isValidating, isFailed, isProcessing, isCancelled } =
        useUploadProgress(activeId, uploadType);
    ...
    const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (allowedExtensions.includes(ext || '')) {
            ...
            toast.error(`Invalid file type. Please upload: ${allowedExtensions.join(', ').toUpperCase()}`);
        }
    };
    ...
    const handleUpload = async () => {
        ...
        try {
            // Trim leading/trailing slashes from apiBasePath
            const cleanPath = apiBasePath.replace(/^\/|\/$/g, '');
    ...
    const handleConfirm = async () => {
        ...
        try {
            ...
            const response = await fetch(
                `${getApiBaseUrl()}/${cleanPath}/${activeId}/confirm`,
    ...
    const handleCancel = async () => {
        ...
        try {
            const cleanPath = apiBasePath.replace(/^\/|\/$/g, '');
            const response = await fetch(
                `${getApiBaseUrl()}/${cleanPath}/${activeId}`,
    ...
    const downloadTemplate = () => {
        const cleanPath = apiBasePath.replace(/^\/|\/$/g, '');
        window.open(`${getApiBaseUrl()}/${cleanPath}/template/download`, '_blank');
    };
    ...
    const downloadErrorReport = () => {
        ...
        const cleanPath = apiBasePath.replace(/^\/|\/$/g, '');
        window.open(
            `${getApiBaseUrl()}/${cleanPath}/${activeId}/error-report`,
            '_blank',
        );
    };
                                        ...
                                        <>
                                            ...
                                            <div className="flex gap-2 mt-2">
                                                {allowedExtensions.map(ext => (
                                                    <Badge key={ext} variant="outline" className="bg-background/50 uppercase">.{ext}</Badge>
                                                ))}
                                            </div>
                                        </>
                                    )}
                        ...
                        <div className="max-w-[300px]">
                            {isProcessing && (
                                <div className="flex items-center gap-2 text-xs text-muted-foreground font-semibold italic">
                                    ...
                                    Processing uploader task in the background...
                                </div>
                            )}
                        </div>

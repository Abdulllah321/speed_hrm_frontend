...
interface AttendanceBulkUploadModalProps {
    open: boolean;
...
export function AttendanceBulkUploadModal({
    open,
    onOpenChange,
    onSuccess,
    uploadId,
    onUploadIdChange
}: AttendanceBulkUploadModalProps) {
    ...
    const {
        ...
        isProcessing,
        isCancelled
    } = useUploadProgress(activeId, 'attendance');
    ...
    const handleUpload = async () => {
        ...
        try {
            const response = await fetch(`${getApiBaseUrl()}/attendances/import-csv`, {
                method: 'POST',
                body: formData,
                credentials: 'include',
            });
    ...
    const handleConfirm = async () => {
        ...
        try {
            const response = await fetch(`${getApiBaseUrl()}/attendances/bulk-upload/${activeId}/confirm`, {
                method: 'POST',
                credentials: 'include',
            });
            const result = await response.json();
            if (result.status !== false) {
                toast.success('Attendance import started');
            } else {
    ...
    const downloadTemplate = () => {
        window.open(`${getApiBaseUrl()}/attendances/import-template`, '_blank');
    };

    const downloadErrorReport = () => {
        if (!activeId) return;
        window.open(`${getApiBaseUrl()}/attendances/bulk-upload/${activeId}/errors/stream`, '_blank');
    };
            ...
            >
                <DialogHeader className="p-6 pb-2 border-b bg-muted/30 shrink-0">
                    ...
                    <DialogDescription className="text-sm">
                        Efficiently record daily attendance. Validate data formats first, then confirm import.
                    </DialogDescription>
                </DialogHeader>
                                    ...
                                    {file ? (
                                        ...
                                        <>
                                            ...
                                            <div className="text-center space-y-2">
                                                <p className="font-bold text-xl">Upload Attendance CSV</p>
                                                <p className="text-sm text-muted-foreground max-w-[300px]">
                                                    Select a CSV or Excel file containing attendance information.
                                                </p>
                                            </div>
                                        </>
                                    )}
                                        ...
                                        <div className="space-y-2">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <p className="text-sm font-bold text-primary uppercase tracking-widest">
                                                    {isValidating ? 'Step 1: Validating Data' : data?.status === 'processing' ? 'Step 2: Importing Attendance' : 'Status'}
                                                </p>
                                            </div>
                                            ...
                                            <div>
                                                ...
                                                <p className="text-sm text-muted-foreground">
                                                    {data?.failedRecords === 0
                                                        ? "All attendance records are valid and ready for import."
                    ...
                    <div className="flex justify-between w-full items-center">
                        ...
                        <div className="flex gap-3">
                            {!activeId ? (
                                ...
                                <>
                                    {isConfirming && data?.status !== 'completed' && data?.status !== 'failed' ? (
                                    ...
                                    ) : isValidated && data?.status === 'validated' && !isConfirming ? (
                                        <div className="flex gap-3">

...
interface CoaBulkUploadModalProps {
    open: boolean;
...
export function CoaBulkUploadModal({
    open,
    onOpenChange,
    onSuccess,
    uploadId,
    onUploadIdChange,
}: CoaBulkUploadModalProps) {
    ...
    const { data, speed, isComplete, isValidated, isValidating, isFailed, isProcessing, isCancelled } =
        useUploadProgress(activeId, 'coa');
    ...
    const handleUpload = async () => {
        ...
        try {
            const response = await fetch(`${getApiBaseUrl()}/finance/chart-of-accounts/bulk-upload`, {
                method: 'POST',
                body: formData,
                credentials: 'include',
            });
            ...
            if (result.status && result.data?.uploadId) {
                ...
                toast.success('File uploaded. Chart of Accounts validation started...');
            } else {
                toast.error(result.message || 'Failed to initiate COA upload');
            }
        } catch (error) {
    ...
    const handleConfirm = async () => {
        ...
        try {
            const response = await fetch(
                `${getApiBaseUrl()}/finance/chart-of-accounts/bulk-upload/${activeId}/confirm`,
                { method: 'POST', credentials: 'include' },
            );
            const result = await response.json();
            if (result.status) {
                toast.success('Chart of Accounts import started');
            } else {
                toast.error(result.message || 'Failed to start COA import');
    ...
    const handleCancel = async () => {
        ...
        try {
            const response = await fetch(
                `${getApiBaseUrl()}/finance/chart-of-accounts/bulk-upload/${activeId}`,
                { method: 'DELETE', credentials: 'include' },
            );
            const result = await response.json();
            if (result.status) {
                toast.info('COA job cancelled');
                onUploadIdChange?.(null);
                setInternalUploadId(null);
            }
        } catch (error) {
    ...
    const downloadTemplate = () => {
        window.open(`${getApiBaseUrl()}/finance/chart-of-accounts/bulk-upload/template/download`, '_blank');
    };

    const downloadErrorReport = () => {
        if (!activeId) return;
        window.open(
            `${getApiBaseUrl()}/finance/chart-of-accounts/bulk-upload/${activeId}/error-report`,
            '_blank',
        );
    };
            ...
            >
                ...
                <DialogHeader className="p-6 pb-2 border-b bg-muted/30 shrink-0">
                    ...
                    <DialogDescription className="text-sm">
                        Upload your Chart of Accounts hierarchy with opening balances. Validate first, then commit to the database.
                    </DialogDescription>
                </DialogHeader>
                    ...
                    <div className="p-6 space-y-6">
                        {!activeId ? (
                            ...
                            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                                    ...
                                    {file ? (
                                        ...
                                        <>
                                            ...
                                            <div className="text-center space-y-2">
                                                <p className="font-bold text-xl">Upload Chart of Accounts</p>
                                                <p className="text-sm text-muted-foreground max-w-[360px]">
                                                    Upload your COA hierarchy (Main → Control → Sub-Control → Leaf) with opening balances.
                                                </p>
                                            </div>
                                ...
                                <div className="flex items-center justify-between p-4 bg-primary/5 rounded-xl border border-primary/10 shadow-sm transition-all hover:shadow-md">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <p className="font-bold text-sm">Download COA Template</p>
                                            <p className="text-xs text-muted-foreground">
                                                Columns: CODE, MAIN/CONTROL/SUB-CONTROL, TAG ID, GL DESCRIPTION, DEBIT, CREDIT
                                            </p>
                                        </div>
                                    </div>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="flex gap-4 p-4 rounded-xl border bg-card/50 transition-colors hover:bg-card">
                                        ...
                                        <div className="space-y-1">
                                            <p className="text-sm font-bold">4-Level Hierarchy</p>
                                            <p className="text-xs text-muted-foreground leading-relaxed">
                                                Main (1-digit) → Control (2-digit) → Sub-Control (4-digit) → Leaf (8-digit)
                                            </p>
                                        </div>
                                    </div>
                                    <div className="flex gap-4 p-4 rounded-xl border bg-card/50 transition-colors hover:bg-card">
                                        ...
                                        <div className="space-y-1">
                                            <p className="text-sm font-bold">Opening Balances</p>
                                            <p className="text-xs text-muted-foreground leading-relaxed">
                                                Debit/Credit opening balances are posted automatically for leaf accounts.
                                            </p>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div className="space-y-8 animate-in fade-in duration-500">
                                        ...
                                        <div className="space-y-2">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <p className="text-sm font-bold text-primary uppercase tracking-widest">
                                                    {isValidating
                                                        ? 'Phase 1: Validating Accounts'
                                                        : data?.status === 'processing'
                                                        ? 'Phase 2: Importing Accounts'
                                                        : 'Status'}
                                                </p>
                                            </div>
                                ...
                                {isValidated && (
                                    <div className="p-6 rounded-2xl border-2 border-dashed bg-card space-y-4 animate-in zoom-in-95 duration-500">
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                <h4 className="font-black text-lg">COA Validation Complete</h4>
                                                <p className="text-sm text-muted-foreground">
                                                    {data?.failedRecords === 0
                                                        ? 'All accounts are valid and ready to be imported.'
                                    ...
                                    <div className="p-8 bg-green-500/5 border-2 border-green-500/20 rounded-3xl flex flex-col items-center gap-4 text-center animate-in zoom-in-95 duration-500">
                                        ...
                                        <div className="space-y-1">
                                            ...
                                            <p className="text-green-600/80 font-medium">
                                                {data?.successRecords} accounts have been added to your Chart of Accounts.
                                            </p>
                                        </div>
                                    </div>
                                )}
                                ...
                                {/* Failed State */}
                                {isFailed && (
                                    <div className="p-8 bg-destructive/5 border-2 border-destructive/20 rounded-3xl flex flex-col items-center gap-4 text-center animate-in zoom-in-95 duration-500">
                                        <div className="h-16 w-16 rounded-full bg-destructive/20 flex items-center justify-center">
                                            <XCircle className="h-10 w-10 text-destructive" />
                                        </div>
                                        <div className="space-y-1">
                                            <h3 className="text-2xl font-black text-destructive">Import Failed</h3>
                                            <p className="text-destructive/80 font-medium">{data?.message}</p>
                                        </div>
                                    </div>
                                )}
                            </div>
                        ...
                        <div className="max-w-[300px]">
                            {isProcessing && (
                                <div className="flex items-center gap-2 text-xs text-muted-foreground font-semibold italic">
                                    ...
                                    Importing accounts in the background...
                                </div>
                            )}
                        </div>

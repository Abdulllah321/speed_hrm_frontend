    ...
    <div className="flex flex-col items-center justify-center h-full min-h-[400px] gap-6 select-none">
      ...
      <div className="text-center space-y-1.5">
        <p className="text-base font-bold tracking-tight">
          Generating Return Slip
        </p>
        <p className="text-sm text-muted-foreground">
          Fetching return details, please wait…
        </p>
      </div>
  ...
  useEffect(() => {
    if (!isLoading && settings.receiptAutoPrint) {
      const timer = setTimeout(
        () => printThermal("return-print-root", settings),
        400,
      );
      ...
      <Dialog open onOpenChange={onClose}>
        <DialogContent className="max-w-2xl w-full h-[92vh] flex flex-col p-0 gap-0">
          <DialogHeader className="px-5 pt-4 pb-3 border-b shrink-0">
            ...
            <p className="text-sm text-muted-foreground">
              {isLoading
                ? `Loading ${isRefund ? "refund" : "return"} details…`
          ...
          <DialogFooter className="px-5 py-3 border-t shrink-0 gap-2">
            ...
            <Button
              onClick={() => printThermal("return-print-root", settings)}
            ...
            >
              {isLoading ? (
                ...
                <>
                  ...
                  {isRefund ? "Print Refund Receipt" : "Print Return Receipt"}
                </>
              )}
            </Button>
          </DialogFooter>
...
function ReturnBody({
  ...
  const isAllianceCase =
    isAlliance ||
    discountNotes?.some((note) => note.toLowerCase().includes("alliance"));
  ...
  return (
    <div className="font-mono text-xs w-full max-w-[72.1mm] mx-auto space-y-2">
      ...
      <div className="space-y-0.5 text-[11px]">
        <Row
          label={isRefund ? "Refund Ref." : "Return Ref."}
          value={returnRef}
          bold
        />
        <Row
          label="Date"
          value={`${fmtDate(returnedAt)} ${fmtTime(returnedAt)}`}
        />
      ...
      <div className="space-y-0.5 text-[11px]">
        ...
        {(() => {
          ...
          const totalDiscount = returnedLines.reduce(
            (s, line) => s + (line.discountAmount ?? 0),
            0,
          );
          const totalTax = returnedLines.reduce(
            (s, line) => s + (line.taxAmount ?? 0),
            0,
          );
          const totalValueIncludingTax = returnedLines.reduce(
            (s, line) => s + (line.refundAmount ?? 0),
            0,
          );
          ...
          return (
            <>
              <Row
                label={`Total Value Excluding Sales Tax (${returnedLines.length})`}
              ...
              />
              <Row
                label="Value for Sales"
                value={fmt(Math.round(valueForSales))}
              />
      ...
      {exchangeVoucher && !isRefund && (
        <>
          ...
          <div className="text-center space-y-1 border-2 border-dashed border-zinc-950 rounded-lg px-3 py-3 bg-zinc-50">
            <p className="font-bold text-xs uppercase tracking-wide text-zinc-900">
              Exchange Voucher Issued
            </p>
            <div className="bg-white border-2 border-zinc-955 rounded px-2 py-2">
              <p className="font-black text-2xl tracking-widest text-zinc-955">
                {exchangeVoucher.code}
              </p>
              <BarcodeImg
                value={exchangeVoucher.code}
                height={32}
                fontSize={8}
              />
            </div>
            <div className="text-[10px] space-y-0.5 pt-1">
              <p className="font-semibold text-zinc-900">
                Value:{" "}
                <span className="font-black text-base text-zinc-955">
                  Rs. {fmt(Number(exchangeVoucher.faceValue))}
                </span>
              </p>
              {exchangeVoucher.expiresAt &&
                fmtExpiryDate(exchangeVoucher.expiresAt) && (

...
interface SalesHistoryBulkUploadModalProps {
    open: boolean;
...
export function SalesHistoryBulkUploadModal({
    open,
    onOpenChange,
    onSuccess,
    uploadId,
    onUploadIdChange,
}: SalesHistoryBulkUploadModalProps) {
    ...
    const { data, speed, isComplete, isValidated, isValidating, isFailed, isProcessing, isCancelled } =
        useUploadProgress(activeId, 'sales-history');
    ...
    const handleUpload = async () => {
        ...
        try {
            const response = await fetch(`${getApiBaseUrl()}/pos-sales/bulk-upload`, {
                method: 'POST',
                body: formData,
                credentials: 'include',
            });
    ...
    const handleConfirm = async () => {
        ...
        try {
            const response = await fetch(`${getApiBaseUrl()}/pos-sales/bulk-upload/${activeId}/confirm`, {
                method: 'POST',
                credentials: 'include',
            });
            const result = await response.json();
            if (result.status) {
                toast.success('Sales history import started');
            } else {
    ...
    const handleCancel = async () => {
        if (!activeId) return;
        try {
            const response = await fetch(`${getApiBaseUrl()}/pos-sales/bulk-upload/${activeId}`, {
                method: 'DELETE',
                credentials: 'include',
            });
            const result = await response.json();
    ...
    const downloadTemplate = () => {
        window.open(`${getApiBaseUrl()}/pos-sales/bulk-upload/template/download`, '_blank');
    };

    const downloadErrorReport = () => {
        if (!activeId) return;
        window.open(`${getApiBaseUrl()}/pos-sales/bulk-upload/${activeId}/error-report`, '_blank');
    };
            ...
            >
                <DialogHeader className="p-6 pb-2 border-b bg-muted/30 shrink-0">
                    ...
                    <DialogDescription className="text-sm">
                        Import previous-year sales history. Rows are grouped by DocumentNumber into orders.
                        Only DocumentNumber and BarCode are required — all other columns are optional.
                    </DialogDescription>
                </DialogHeader>
                                    ...
                                    {file ? (
                                        ...
                                        <>
                                            ...
                                            <div className="text-center space-y-2">
                                                <p className="font-bold text-xl">Upload Sales History</p>
                                                <p className="text-sm text-muted-foreground max-w-[360px]">
                                                    Upload your CSV or Excel file. Rows with the same DocumentNumber
                                                    are grouped into a single order automatically.
                                                </p>
                                            </div>
                                    ...
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs text-muted-foreground">
                                                Required columns: DocumentNumber, DocumentDate, BarCode, Quantity, UnitPrice…
                                            </p>
                                        </div>
                                    </div>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="flex gap-4 p-4 rounded-xl border bg-card/50 hover:bg-card transition-colors">
                                        ...
                                        <div className="space-y-1">
                                            <p className="text-sm font-bold">Order Grouping</p>
                                            <p className="text-xs text-muted-foreground leading-relaxed">
                                                Multiple rows with the same DocumentNumber are merged into one order.
                                            </p>
                                        </div>
                                    </div>
                                    <div className="flex gap-4 p-4 rounded-xl border bg-card/50 hover:bg-card transition-colors">
                                        ...
                                        <div className="space-y-1">
                                            <p className="text-sm font-bold">BarCode Lookup</p>
                                            <p className="text-xs text-muted-foreground leading-relaxed">
                                                Items are matched by BarCode. Scientific notation (4.06E+12) is handled automatically.
                                            </p>
                                        </div>
                                    </div>
                                </div>
                                        ...
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-sm text-muted-foreground">
                                                    {data?.failedRecords === 0
                                                        ? 'All rows are valid. Starting import automatically...'
                                                    ...
                                                    <Table>
                                                        <TableHeader className="bg-muted/50 sticky top-0 z-10 backdrop-blur-sm">
                                                            <TableRow>
                                                                ...
                                                                <TableHead className="w-[120px] font-black uppercase text-[10px]">Doc #</TableHead>
                                                        ...
                                                        <TableBody>
                                                            {data.errors.slice(0, 100).map((err, i) => (
                                                                <TableRow key={i} className="hover:bg-muted/20 transition-colors">
                                                                    ...
                                                                    <TableCell className="text-xs font-bold">{err.data?.documentNumber || '—'}</TableCell>
                                                                    ...
                                                                    <TableCell className="text-right">
                                                                        <Badge variant="outline" className="text-[10px] font-mono font-bold bg-background">
                                                                            {String(err.data?.barCode || err.data?.value || 'N/A')}
                                                                        </Badge>
                                                                    </TableCell>
                                                                </TableRow>
                                                            ))}
                                    ...
                                    <div className="p-8 bg-green-500/5 border-2 border-green-500/20 rounded-3xl flex flex-col items-center gap-4 text-center animate-in zoom-in-95 duration-500">
                                        ...
                                        <div className="space-y-1">
                                            ...
                                            <p className="text-green-600/80 font-medium">
                                                {data?.successRecords} sales records have been imported.
                                            </p>
                                        </div>
                                    </div>
                                )}
                        ...
                        <div className="max-w-[300px]">
                            {isProcessing && (
                                <div className="flex items-center gap-2 text-xs text-muted-foreground font-semibold italic">
                                    ...
                                    Processing in background — you can close this window.
                                </div>
                            )}
                        </div>
                                ...
                                <>
                                    ...
                                    >
                                        {isUploading ? (
                                            ...
                                            <><Upload className="h-4 w-4 mr-2" /> Start Validation</>
                                        )}
                                    </Button>
                                </>
                            ) : (
                                        ...
                                        <div className="flex gap-3">
                                            ...
                                            >
                                                {isConfirming ? (
                                                    ...
                                                    <><Database className="mr-2 h-4 w-4" /> Confirm & Import</>
                                                )}
                                            </Button>
                                        </div>
                                    ) : (

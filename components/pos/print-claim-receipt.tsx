    ...
    <div className="flex flex-col items-center justify-center h-full min-h-[400px] gap-6 select-none">
      ...
      <div className="text-center space-y-1.5">
        <p className="text-base font-bold tracking-tight">
          Generating Claim Receipt
        </p>
        <p className="text-sm text-muted-foreground">
          Fetching claim details, please wait…
        </p>
      </div>
  ...
  useEffect(() => {
    if (!isLoading && settings.receiptAutoPrint) {
      const timer = setTimeout(
        () => printThermal("claim-print-root", settings),
        400,
      );
      ...
      <Dialog open onOpenChange={onClose}>
        <DialogContent className="max-w-2xl w-full h-[92vh] flex flex-col p-0 gap-0">
          ...
          <DialogFooter className="px-5 py-3 border-t shrink-0 gap-2">
            ...
            <Button
              onClick={() => printThermal("claim-print-root", settings)}
  ...
  return (
    <div className="font-mono text-xs w-full max-w-[72.1mm] mx-auto space-y-2">
      ...
      <div className="space-y-0.5 text-[11px]">
        ...
        <Row
          label="Submitted"
          value={`${fmtDate(submittedAt)}, ${fmtTime(submittedAt)}`}
        />
        {reviewedAt && (
          <Row
            label="Reviewed"
            value={`${fmtDate(reviewedAt)}, ${fmtTime(reviewedAt)}`}
          />
        )}
      </div>
      ...
      {reasonCode && (
        <>
          ...
          <div className="space-y-0.5 text-[11px]">
            ...
            {reasonNotes && (
              <p style={{ paddingLeft: "8px", fontSize: "10px" }}>
                {reasonNotes}
              </p>
            )}
          </div>
        ...
        return (
          <div key={idx} className="pb-2 border-b border-dashed last:border-0">
            ...
            <div className="mt-1 space-y-0.5 text-[10px]">
              <Row
                label="Unit Paid Price"
                value={`Rs. ${fmt(line.unitPaidPrice)}`}
              />
              {line.itemStatus && (
                <div
                  style={{
      ...
      <Separator />
      <div
        className="text-[11px] space-y-1 border border-dashed rounded px-2 py-2"
        style={{ borderColor: statusColor }}
      >
        {isApproved && (
          <>
            <p className="font-bold text-center" style={{ color: statusColor }}>
              ✓ CLAIM APPROVED ✓
            </p>
            <p className="text-center">
              Please present this receipt to collect your refund.
            </p>

            {/* ── Exchange Voucher Details ── */}
            {voucher && voucher.voucherType === "EXCHANGE" && (
              <div
                className="mt-2 pt-2 border-t border-dashed space-y-1"
                style={{ borderColor: statusColor }}
              >
                <p
                ...
                <div className="space-y-0.5 text-[10px] bg-green-50 dark:bg-green-950/20 p-2 rounded">
                  ...
                  <Row
                    ...
                    value={`Rs. ${fmt(Number(voucher.faceValue))}`}
                    bold
                  />
                  <Row label="Valid Until" value={fmtDate(voucher.expiresAt)} />
                  <Row
                    label="Status"
                    value={voucher.isRedeemed ? "USED" : "ACTIVE"}
                  />
                </div>
                <p className="text-center text-[10px] mt-1">
                  Use this voucher code for your next purchase!
                </p>
              </div>
            )}
          </>
          ...
          <>
            ...
            {reviewNotes && (
              ...
              >
                ...
                <p className="text-center text-[9px] leading-tight px-1">
                  {reviewNotes}
                </p>
              </div>
            )}
        ...
        {isPending && (
          <>
            ...
            <p className="text-center">
              Your claim is being reviewed. Please check back later.
            </p>
            ...
            >
              ...
              <p className="text-center text-[9px] leading-tight">
                This receipt does NOT guarantee claim acceptance. The final
                decision rests with Product Line Management. Your product has
                been sent for inspection.
              </p>
              <p className="text-center text-[9px] leading-tight mt-1">
                Acceptance/rejection of this claim will be at the sole
                discretion of Product Line.
              </p>
            </div>
          </>
        )}

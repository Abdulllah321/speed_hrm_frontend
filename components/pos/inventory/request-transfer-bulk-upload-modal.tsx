...
export interface RequestTransferImportItem {
    id: string;
    sku: string;
    description: string;
    size?: string;
...
interface RowResult {
    ...
    color?: string;
    maxQuantity?: number;
    /** Error / reason when not found or warning note */
    reason?: string;
}
...
interface Props {
    ...
    /** Called with matched items to add to the request list */
    onImportComplete: (items: RequestTransferImportItem[]) => void;
}
...
export function RequestTransferBulkUploadModal({
    open,
    onOpenChange,
    fromWarehouseId,
    fromLocationId,
    ...
    const handleStart = useCallback(async () => {
                ...
                if (res.status && res.data && res.data.length > 0) {
                    ...
                    if (prodStock <= 0) {
                        results[i] = {
                            ...results[i],
                            status: 'error',
                            reason: 'Out of Stock at Sourcing Location',
                        };
                    } else {
                        // Check if the requested quantity exceeds stock
                        let finalQty = row.quantity;
                        let reason = '';
                        if (row.quantity > prodStock) {
                            finalQty = prodStock;
                            reason = `Capped: ${prodStock} units available at source`;
                        }
                    ...
                    results[i] = {
                        ...results[i],
                        status: 'not_found',
                        reason: 'Item not found at sourcing location',
                    };
                }
            } catch (err: any) {
                results[i] = {
        ...
        if (matched.length === 0) {
            toast.error('No items were found at source. Check your barcodes.');
        } else {
            toast.success(`${matched.length} item(s) matched and ready to import.`);
        }
    }, [file, fromWarehouseId, fromLocationId]);
    ...
    const handleConfirm = () => {
        ...
        const items: RequestTransferImportItem[] = matched.map((r) => ({
            id: r.id!,
            sku: r.sku!,
    ...
    const handleDownloadErrorReport = () => {
        ...
        const ws = XLSX.utils.json_to_sheet(
            errors.map((r) => ({
                ...
                BarCode: r.barCode,
                Quantity: r.quantity,
                Reason: r.reason || 'Not found at source',
            }))
        );
        const wb = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(wb, ws, 'Errors');
        XLSX.writeFile(wb, 'request_transfer_import_errors.xlsx');
    };
    ...
    const handleDownloadTemplate = () => {
        ...
        link.setAttribute('download', 'request_transfer_template.csv');
            ...
            >
                ...
                <DialogHeader className="p-6 pb-3 border-b bg-muted/30 shrink-0">
                    <DialogTitle className="flex items-center gap-2 text-xl font-bold tracking-tight">
                        ...
                        Import Items for Request Transfer
                        {phase !== 'select' && (
                    ...
                    <DialogDescription className="text-sm">
                        Upload a CSV or Excel file with <span className="font-mono font-bold text-emerald-600">barCode</span> and <span className="font-mono font-bold text-emerald-600">quantity</span> columns.
                        Valid items available at source location will be added to the request list.
                    </DialogDescription>
                </DialogHeader>
                            ...
                            <div className="space-y-5 animate-in fade-in slide-in-from-bottom-4 duration-400">
                                ...
                                <div
                                    ...
                                    className={cn(
                                        ...
                                        file
                                            ...
                                            : 'border-muted-foreground/20 hover:border-emerald-500/40 hover:bg-muted/50'
                                    )}
                                >
                                    <input
                                        type="file"
                                    ...
                                    {file ? (
                                        <>
                                            <div className="h-16 w-16 rounded-2xl bg-emerald-500/10 flex items-center justify-center shadow-inner group-hover:scale-105 transition-transform">
                                        ...
                                        <>
                                            <div className="h-16 w-16 rounded-2xl bg-muted/50 flex items-center justify-center group-hover:bg-emerald-500/5 transition-colors">
                                                <Upload className="h-8 w-8 text-muted-foreground group-hover:text-emerald-600 transition-colors" />
                                            </div>
                                ...
                                <div className="flex items-center justify-between p-4 bg-muted/20 border rounded-2xl">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <h4 className="font-bold text-sm">Download Template</h4>
                                            <p className="text-xs text-muted-foreground">Ensure your data matches the system requirements.</p>
                                        </div>
                                    </div>
                                    <Button variant="outline" size="sm" onClick={handleDownloadTemplate} className="font-semibold text-xs h-9">
                                        Get CSV Template
                                    </Button>
                                </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div className="flex items-start gap-3 p-4 bg-muted/10 border rounded-2xl">
                                        <div className="p-2 bg-emerald-500/10 text-emerald-600 rounded-lg shrink-0">
                                            <CheckCircle2 className="h-5 w-5" />
                                        </div>
                                        <div>
                                            <h4 className="font-bold text-xs">Smart Validation</h4>
                                            <p className="text-[11px] text-muted-foreground mt-0.5 leading-relaxed">
                                                Instantly identifies formatting errors and duplicates before DB entry.
                                            </p>
                                        </div>
                                    </div>
                                    <div className="flex items-start gap-3 p-4 bg-muted/10 border rounded-2xl">
                                        ...
                                        <div>
                                            <h4 className="font-bold text-xs">SSE Streaming</h4>
                                            <p className="text-[11px] text-muted-foreground mt-0.5 leading-relaxed">
                                                Real-time progress updates with instant feedback on every row.
                                            </p>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div className="space-y-6 animate-in fade-in duration-400">
                                    ...
                                    <div className="flex justify-between items-end">
                                        <div className="space-y-1">
                                            <div className="flex items-center gap-2">
                                                {isRunning && <Loader2 className="h-4 w-4 animate-spin text-emerald-600" />}
                                ...
                                <div className="grid grid-cols-4 gap-3">
                                    ...
                                    <div className="bg-green-500/10 p-4 rounded-2xl border border-green-500/20 flex flex-col items-center justify-center shadow-sm">
                                        <span className="text-[10px] font-black text-green-600 uppercase tracking-widest mb-1">Matched</span>
                                    ...
                                    <div className="bg-destructive/10 p-4 rounded-2xl border border-destructive/20 flex flex-col items-center justify-center shadow-sm">
                                        <span className="text-[10px] font-black text-destructive uppercase tracking-widest mb-1">Failed</span>
                                                ...
                                                {rows.map((row, i) => (
                                                    ...
                                                    >
                                                        ...
                                                        <TableCell className="text-xs text-muted-foreground max-w-[120px] truncate">
                                                            {row.reason || (row.status === 'found' ? `Source Stock: ${row.maxQuantity}` : '')}
                                                        </TableCell>
                                                    </TableRow>
                                                ))}
                                                <tr>
                                    ...
                                    <div className="p-5 bg-green-500/5 border-2 border-green-500/20 rounded-2xl flex items-center gap-4 animate-in zoom-in-95 duration-400">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-sm text-green-600/80">
                                                Click <strong>Apply to Transfer</strong> to add these items.
                                                {notFound > 0 && ` (${notFound} barcode(s) failed / not found will be skipped.)`}
                                            </p>
                                        </div>
                                    </div>
                                )}
                                    ...
                                    <div className="p-5 bg-destructive/5 border-2 border-destructive/20 rounded-2xl flex items-center gap-4 animate-in zoom-in-95 duration-400">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-sm text-destructive/80">
                                                None of the barcodes could be resolved with available stock at the source location. Check the file and try again.
                                            </p>
                                        </div>
                                    </div>
                                )}
                            ...
                            {phase === 'select' && (
                                <>
                                    ...
                                    <Button
                                        disabled={!file}
                                        onClick={handleStart}
                                        className="px-8 font-black bg-emerald-600 hover:bg-emerald-700 text-white shadow-lg shadow-emerald-600/20"
                                    >
                            ...
                            {isDone && (
                                <>
                                    ...
                                    <Button
                                        ...
                                        disabled={found === 0}
                                        className="px-8 font-black bg-emerald-600 hover:bg-emerald-700 text-white shadow-lg shadow-emerald-600/20 disabled:opacity-50"
                                    >
                                        <CheckCircle2 className="h-4 w-4 mr-2" />
                                        Apply to Transfer ({found} items)
                                    </Button>
                                </>
                            )}
                        </div>

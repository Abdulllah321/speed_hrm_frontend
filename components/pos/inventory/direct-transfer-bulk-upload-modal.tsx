...
export interface DirectTransferImportItem {
    id: string;
    sku: string;
    description: string;
    size?: string;
...
interface Props {
    ...
    /** Called with matched items to add to direct transfer list */
    onImportComplete: (items: DirectTransferImportItem[]) => void;
}
...
export function DirectTransferBulkUploadModal({ open, onOpenChange, onImportComplete }: Props) {
    ...
    const handleStart = useCallback(async () => {
        ...
        for (let i = 0; i < parsed.length; i++) {
            ...
            try {
                const res = await authFetch(`/pos-sales/scan`, { params: { barcode: row.barCode } });
                if (res.ok && res.data?.status && res.data.data) {
                    ...
                    if (prodStock <= 0) {
                        results[i] = {
                            ...results[i],
                            status: 'error',
                            reason: 'Out of Stock (Available: 0)',
                        };
                    } else {
                        ...
                        if (row.quantity > prodStock) {
                            finalQty = prodStock;
                            reason = `Capped: ${prodStock} units available`;
                        }
                    ...
                    results[i] = {
                        ...results[i],
                        status: 'not_found',
                        reason: res.data?.message || 'No item found for this barcode',
                    };
                }
            } catch (err: any) {
                results[i] = {
    ...
    const handleConfirm = () => {
        ...
        const items: DirectTransferImportItem[] = matched.map((r) => ({
            id: r.id!,
            sku: r.sku!,
    ...
    const handleDownloadErrorReport = () => {
        ...
        XLSX.writeFile(wb, 'direct_transfer_import_errors.xlsx');
    };
    ...
    const handleDownloadTemplate = () => {
        ...
        link.setAttribute('download', 'direct_transfer_template.csv');
            ...
            >
                ...
                <DialogHeader className="p-6 pb-3 border-b bg-muted/30 shrink-0">
                    ...
                    <DialogDescription className="text-sm">
                        ...
                        Valid stock items will be appended/merged into your direct transfer list.
                    </DialogDescription>
                </DialogHeader>
                                                ...
                                                {rows.map((row, i) => (
                                                    ...
                                                    >
                                                        ...
                                                        <TableCell className="text-xs text-muted-foreground max-w-[120px] truncate">
                                                            {row.reason || (row.status === 'found' ? `Stock: ${row.availableStock}` : '')}
                                                        </TableCell>
                                                    </TableRow>
                                                ))}
                                                <tr>
                                    ...
                                    <div className="p-5 bg-destructive/5 border-2 border-destructive/20 rounded-2xl flex items-center gap-4 animate-in zoom-in-95 duration-400">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-sm text-destructive/80">
                                                None of the barcodes could be resolved with available stock. Check the file and try again.
                                            </p>
                                        </div>
                                    </div>
                                )}

    ...
    <div className="flex flex-col items-center justify-center h-full min-h-[400px] gap-6 select-none">
      ...
      <div className="text-center space-y-1.5">
        <p className="text-base font-bold tracking-tight">
          Generating Voucher Receipt
        </p>
        <p className="text-sm text-muted-foreground">
          Preparing voucher details…
        </p>
      </div>
  ...
  useEffect(() => {
    if (!isLoading && autoPrint && voucherList.length > 0) {
      const timer = setTimeout(
        () => printThermal("voucher-receipt-print-root", settings),
        400,
      );
            ...
            >
              {isLoading ? (
                ...
                <>
                  ...
                  {isBulk
                    ? `Print ${voucherList.length} Vouchers`
                    : "Print Voucher"}
                </>
              )}
            </Button>
  ...
  return (
    <div className="font-mono text-xs w-full max-w-[72.1mm] mx-auto space-y-1.5">
      ...
      <div className="text-center space-y-2 py-3 border border-dashed border-zinc-950 rounded bg-zinc-50">
        <p className="text-[9px] text-zinc-800 uppercase tracking-wide">
          Voucher Code
        </p>
        <p className="font-black text-xl tracking-widest text-zinc-955">
          {voucher.code}
        </p>
      ...
      <div className="text-center py-2 bg-zinc-100 rounded border border-zinc-200">
        ...
        <p className="font-black text-2xl tracking-wider text-zinc-955">
      ...
      <div className="space-y-0.5 text-[11px]">
        <Row
          label="Issued On"
          value={`${fmtDate(voucher.createdAt)} ${fmtTime(voucher.createdAt)}`}
        />
        <Row
          label="Expires On"
          value={voucher.expiresAt ? fmtDate(voucher.expiresAt) : "No Expiry"}
        />
        ...
        {voucher.sourceOrder?.orderNumber && (
          <Row
            ...
            value={voucher.sourceOrder.orderNumber}
          />
        )}
        {voucher.sourceOrder?.returnNumber && (
          <Row
            label="Against Return"
            value={voucher.sourceOrder.returnNumber}
          />
        )}
        {voucher.sourceOrder?.refundNumber && (
          <Row
            label="Against Refund"
            value={voucher.sourceOrder.refundNumber}
          />
        )}

        {terminalName && <Row label="Terminal" value={terminalName} />}
        {voucher.companyName && (
          <Row label="Company" value={voucher.companyName} />
        )}
        {voucher.customer && (
          <Row
            label="Customer"
      ...
      <div className="space-y-0.5">
        ...
        {isUnrestricted ? (
          ...
          <ul
            className="space-y-0"
            style={{
              columns: locationNames.length > 5 ? 2 : 1,
              columnGap: "4px",
            }}
          >

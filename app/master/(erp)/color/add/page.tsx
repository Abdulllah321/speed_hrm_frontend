...
import { createColors } from "@/lib/actions/color";
...
export default function AddColorPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        startTransition(async () => {
            const result = await createColors(validItems.map(i => ({ name: i.name, status: i.status })));
            if (result.status) {
                toast.success(result.message || "Created successfully");
                startTransition(() => {
                    addTransitionType("nav-back");
                    router.push("/master/color/list");
                });
            } else {
    ...
    return (
        <PermissionGuard permissions="master.color.create">
            <div className="max-w-4xl mx-auto">
                <div className="mb-6">
                    <Link href="/master/color/list" transitionTypes={["nav-back"]}>
                ...
                <Card>
                    <CardHeader>
                        <CardTitle>Add Colors</CardTitle>
                        <CardDescription>Create one or more colors</CardDescription>
                    </CardHeader>
                    <CardContent>

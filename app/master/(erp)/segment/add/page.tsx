...
import { createSegments } from "@/lib/actions/segment";
...
export default function AddSegmentPage() {
    ...
    const onSubmit = async (e: React.FormEvent) => {
        ...
        if (validItems.length === 0) {
            toast.error("Please add at least one segment with a name");
            return;
        }

        startTransition(async () => {
            const result = await createSegments(validItems);
            if (result.status) {
                toast.success(result.message);
                router.push("/master/segment/list");
            } else {
            ...
            <div className="p-6">
                <form onSubmit={onSubmit}>
                    <Card className="max-w-4xl mx-auto">
                        <CardHeader>
                            <CardTitle>Add New Segments</CardTitle>
                            <CardDescription>Add one or more segments in bulk.</CardDescription>
                        </CardHeader>
                                ...
                                <div key={index} className="flex flex-col gap-4 p-4 border rounded-lg relative bg-muted/30">
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div className="space-y-2">
                                            <Label htmlFor={`name-${index}`}>Segment Name</Label>
                                            <Input
                                                id={`name-${index}`}

...
export function LoanTypeList({
  ...
  const canBulkEdit = hasPermission("loan-type.update");
  const canBulkDelete = hasPermission("loan-type.delete");
  ...
  const handleMultiDelete = (ids: string[]) => {
    startTransition(async () => {
      const result = await deleteLoanTypes(ids);
      if (result.status) {
  ...
  const handleBulkEdit = (items: LoanTypeRow[]) => {
    setEditRows(
      items.map((item) => ({
        id: item.id,
        name: item.name,
  ...
  const handleBulkEditSubmit = async () => {
    ...
    startTransition(async () => {
      const result = await updateLoanTypes(validRows);
      if (result.status) {
        toast.success(result.message);
  ...
  const data: LoanTypeRow[] = initialLoanTypes.map((item) => ({
    ...item,
    id: item.id.toString(),
  }));
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">
          Manage loan types for your organization
        </p>
      </div>
      ...
      <DataTable<LoanTypeRow>
        ...
        tableId="loan-type-list"
      />

      <Dialog open={bulkEditOpen} onOpenChange={setBulkEditOpen}>
        <DialogContent className="max-w-xl">
          <DialogHeader>
            <DialogTitle>Edit Loan Types</DialogTitle>
            <DialogDescription>
              Update {editRows.length} loan type(s)
            </DialogDescription>
          </DialogHeader>

...
interface MaritalStatusListProps {
  initialMaritalStatuses: MaritalStatus[];
  newItemId?: string;
}

export function MaritalStatusList({ initialMaritalStatuses, newItemId }: MaritalStatusListProps) {
  ...
  const handleMultiDelete = (ids: string[]) => {
    startTransition(async () => {
      const result = await deleteMaritalStatuses(ids);
      if (result.status) {
  ...
  const handleBulkEdit = (items: MaritalStatusRow[]) => {
  ...
  const handleBulkEditSubmit = async () => {
    ...
    startTransition(async () => {
      const result = await updateMaritalStatuses(validRows);
      if (result.status) {
        toast.success(result.message);
  ...
  const data: MaritalStatusRow[] = initialMaritalStatuses.map((ms) => ({
    ...ms,
    id: ms.id.toString(),
  }));
  ...
  return (
    <div className="space-y-6">
      ...
      <DataTable<MaritalStatusRow>
        columns={columns}
        data={data}
        actionText={showAddAction ? "Add Marital Status" : undefined}
      ...
      <Dialog open={bulkEditOpen} onOpenChange={setBulkEditOpen}>
        <DialogContent className="max-w-xl">
          <DialogHeader>
            <DialogTitle>Edit Marital Statuses</DialogTitle>
            <DialogDescription>Update {editRows.length} marital status(es)</DialogDescription>
          </DialogHeader>

...
function RequestTransferForm() {
  ...
  const handleCsvImportComplete = useCallback(
    (importedItems: RequestTransferImportItem[]) => {
      setItems((prev) => {
        const updated = [...prev];
  ...
  const handleSelectProduct = (product: any) => {
    ...
    if (prodStock <= 0) {
      toast.warning(
        `Item ${product.sku} has no available stock at the source location.`,
      );
    }
    ...
    setItems((prev) =>
      prev.map((item) => {
        ...
        if (val > item.maxQuantity) {
          toast.error(
            `Cannot request more than available stock at source (${item.maxQuantity} units)`,
          );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    for (const item of items) {
      ...
      if (item.quantity > item.maxQuantity) {
        toast.error(
          `Quantity for ${item.sku} exceeds available stock at source (${item.maxQuantity} units)`,
        );
        return;
      }
    }
  ...
  const handlePrintCreatedRequest = () => {
    ...
    const refNo =
      createdRequest.requestNo || createdRequest.requisitionNo || "N/A";
    const isOutletToOutlet =
      createdRequest.transferType === "OUTLET_TO_OUTLET" ||
      createdRequest.documentType === "OUTLET_TO_OUTLET";
    const typeTitle = isOutletToOutlet
      ? "OUTLET-TO-OUTLET TRANSFER REQUEST"
      ...
      <div className="flex flex-col items-center justify-center min-h-[70vh] p-6 text-center">
        <div className="max-w-md p-6 bg-card border rounded-2xl shadow-xl space-y-4">
          ...
          <p className="text-muted-foreground text-sm">
            Please go to the Outlet Request page and select a sourcing location
            to request stock.
          </p>
        ...
        <div className="flex items-center gap-4 max-w-5xl mx-auto w-full">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-muted-foreground">
              Request stock from warehouse or another outlet
            </p>
          </div>
                    ...
                    {createdRequest && (
                      <div className="p-4 bg-muted/40 rounded-xl w-full border font-mono text-xs space-y-3 text-left">
                        ...
                        <div className="flex justify-between">
                          ...
                          <span className="font-bold text-foreground">
                            {createdRequest.transferType ||
                              createdRequest.documentType ||
                              (fromLocationId
                                ? "OUTLET_TO_OUTLET"
                                : "Outlet Request")}
                          </span>
                      ...
                      {items.length === 0 ? (
                        <div className="text-center py-12 text-muted-foreground/60 space-y-3">
                          ...
                          <p className="text-xs text-muted-foreground/80">
                            Use the search bar above to query and add items from
                            the source location.
                          </p>
                        </div>
                      ) : (
                            ...
                            >
                              <div className="flex-1 min-w-0 space-y-1">
                                ...
                                <div className="text-[10px] text-blue-600 dark:text-blue-400 font-semibold uppercase tracking-wider">
                                  Available Stock at Source: {item.maxQuantity}{" "}
                                  Units
                                </div>
                              </div>
                    ...
                    <CardContent className="p-6">
                      <form onSubmit={handleSubmit} className="space-y-5">
                        <div className="p-4 bg-blue-50/50 dark:bg-blue-950/20 rounded-xl border border-blue-100 dark:border-blue-900/50 space-y-3">
                          <div>
                            <span className="text-[10px] font-bold text-blue-600 dark:text-blue-400 uppercase block tracking-wider mb-0.5">
                              Sourcing Location
                            </span>
                        ...
                        <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl p-4 flex gap-3 text-amber-700 dark:text-amber-400 text-xs font-medium">
                          ...
                          <p className="leading-relaxed">
                            This request will be sent to the sourcing branch
                            manager for approval. Stock will be reserved upon
                            approval and notified to your terminal.
                          </p>
                        </div>

...
export default function CheckoutPage() {
  ...
  const [promoScopedItems, setPromoScopedItems] = useState<Set<string>>(
    new Set(),
  );
  const [showPromoScope, setShowPromoScope] = useState(false);

  const [selectedAlliance, setSelectedAlliance] =
    useState<AllianceConfig | null>(null);
  const [allianceMeta, setAllianceMeta] = useState({
    cardholderName: "",
    cardLast4: "",
    merchantSlip: "",
    binNumber: "",
  ...
  const [pendingManualDiscountValue, setPendingManualDiscountValue] =
    useState<number>(0);
  ...
  const [selectedMerchant, setSelectedMerchant] =
    useState<MerchantConfig | null>(null);
  ...
  const [appliedVouchers, setAppliedVouchers] = useState<
    { voucherId: string; code: string; amount: number }[]
  >([]);
  ...
  useEffect(() => {
    ...
    const recalculatedItems = items.map((item) => {
      ...
      const discountPercent =
        item.overrideDiscountPercent ?? item.discountPercent;
  ...
  const hasFbrInfo = !!(
    user?.terminal?.location?.fbrEnabled && user?.terminal?.location?.fbrNtn
  );
  ...
  if (discountMode === "alliance" && selectedAlliance) {
    ...
    const scopedSubtotal = promoScopeAll
      ? subtotalAfterItems
      : cartItems
          .filter((i) => promoScopedItems.has(i.id))
  ...
  const handleValidateCoupon = useCallback(async () => {
    ...
    setCouponError("");
    try {
      const res = await authFetch("/pos-config/validate-coupon", {
        method: "POST",
        body: {
          code: couponInput.trim().toUpperCase(),
          orderSubtotal: subtotalAfterItems,
        },
      });
      if (res.ok && res.data?.status) {
        ...
        toast.success(
          `Coupon "${res.data.data.code}" — ${fmtCurrency(res.data.data.discountAmount)} off`,
        );
      } else {
  ...
  const addTender = () => {
    ...
    if (discountMode === "alliance" && selectedAlliance) {
      ...
      if (
        ...
        selectedAlliance.binNumbers.length > 0 &&
    ...
    if (
      ...
      toast.error(
        "Please select a merchant / bank terminal before adding a card payment.",
      );
      return;
    }
  ...
  const addVoucherTender = () => {
    ...
    if (tenderAmount > maxAllowed) {
      toast.error(
        `Voucher amount exceeds the invoice total of ${fmtCurrency(grandTotal)}. Maximum allowed: ${fmtCurrency(maxAllowed)}.`,
      );
      return;
    }
  ...
  const removeTender = (i: number) => {
    ...
    if (removed.method === "voucher") {
      setAppliedVouchers((prev) =>
        prev.filter(
          (v) => v.code !== removed.slipNo && v.voucherId !== removed.voucherId,
        ),
      );
    }
  ...
  const handleHold = useCallback(
    async (holdUntilTime?: string) => {
      ...
      try {
        ...
        const res = await authFetch("/pos-sales/orders/hold", {
          method: "POST",
          body: payload,
        });
        if (res.ok && res.data?.status) {
  ...
  const handleConfirm = useCallback(async () => {
    ...
    if (hasCardTender && !selectedMerchant) {
      toast.error(
        "Please select a merchant / bank terminal for the card payment.",
      );
      return;
    }
    setIsSubmitting(true);
    try {
      const orderItems = cartItems.map((item) => ({
        ...
        promoDiscountAmount:
          ...
          selectedPromo &&
          !promoScopeAll &&
          promoScopedItems.has(item.id)
            ? Math.round(
                calcPromoDiscount(selectedPromo, item.total) /
                  (promoScopedItems.size || 1),
              )
            : 0,
      }));
      ...
      if (
        selectedMerchant &&
        tenders.some((t) => t.method === "card" || t.method === "bank_transfer")
      ) {
  ...
  const handleCreditSale = useCallback(async () => {
    ...
    // Merchant required if any card / bank_transfer tender was added
    const hasCardTenderCredit = tenders.some(
      (t) => t.method === "card" || t.method === "bank_transfer",
    );
    ...
    if (
      !confirm(
        `Confirm credit sale of ${fmtCurrency(grandTotal)} to ${selectedCustomer.name}?\n\nBalance will be added to customer ledger.`,
      )
    )
      return;
    ...
    try {
      ...
      toast.error("Credit sale failed. Check connection.");
    } finally {
      setIsSubmitting(false);
    }
  }, [
    ...
    const handler = (e: KeyboardEvent) => {
      ...
      if (isAlt1To5 || isCtrl1To5) {
        ...
        if (
          ...
          toast.error(
            "Cash payment is disabled when an Alliance discount is selected.",
          );
          return;
        }
        ...
        toast.info(
          `Payment method set to: ${method.replace("_", " ").toUpperCase()}`,
        );
      ...
      if (e.key === "F7" || (e.altKey && (e.key === "m" || e.key === "M"))) {
        e.preventDefault();
        const manualDiscInput = document
          .querySelector("details summary:has(.lucide-wallet)")
          ?.closest("details");
        if (manualDiscInput) {
          manualDiscInput.setAttribute("open", "");
          setTimeout(() => {
            const valInput = manualDiscInput.querySelector(
              'input[type="number"]',
            );
            if (valInput) {
      ...
      if (e.key === "F8" || (e.altKey && (e.key === "p" || e.key === "P"))) {
        e.preventDefault();
        const promoCampaigns = document
          .querySelector("details summary:has(.lucide-tag)")
          ?.closest("details");
        if (promoCampaigns) {
    ...
    <>
        ...
        <PrintReceipt
          order={{
            ...
            alliance: selectedAlliance
              ? { code: selectedAlliance.code }
              : undefined,
            cashierName:
              cashiers.find((c: any) => c.userId === selectedCashierId)?.name ||
              "",
          }}
          cartItems={cartItems}
          tenders={
      ...
      >
        ...
        <div className="p-4 space-y-3.5 max-h-[350px] overflow-y-auto text-xs">
          <div>
            <p className="font-bold text-[10px] uppercase tracking-wider text-primary mb-1.5 text-left">
              Navigation & Customer
            </p>
            <div className="space-y-1.5 font-medium">
              ...
              <div className="flex justify-between items-center">
                <span className="text-muted-foreground">Select Salesman</span>
                <kbd className="px-1.5 py-0.5 bg-muted border rounded text-[10px] font-mono">
                  Alt + A
                </kbd>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-muted-foreground">Add New Customer</span>
                <kbd className="px-1.5 py-0.5 bg-muted border rounded text-[10px] font-mono">
                  Alt + N
                </kbd>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-muted-foreground">Go Back to Cart</span>
                <kbd className="px-1.5 py-0.5 bg-muted border rounded text-[10px] font-mono">
                  Esc
                </kbd>
              </div>
          ...
          <div className="border-t pt-2.5">
            ...
            <div className="space-y-1.5 font-medium">
              ...
              <div className="flex justify-between items-center">
                <span className="text-muted-foreground">Focus Pay Amount</span>
                <kbd className="px-1.5 py-0.5 bg-muted border rounded text-[10px] font-mono">
                  F4
                </kbd>
              </div>
          ...
          <div className="border-t pt-2.5">
            ...
            <div className="space-y-1.5 font-medium">
              ...
              <div className="flex justify-between items-center">
                <span className="text-muted-foreground">Preview Receipt</span>
                <kbd className="px-1.5 py-0.5 bg-muted border rounded text-[10px] font-mono">
                  Alt + V
                </kbd>
              </div>
          ...
          <div className="border-t pt-2.5 text-[10px] text-muted-foreground text-center">
            Press <span className="font-bold text-foreground">F1</span> or{" "}
            <span className="font-bold text-foreground">Ctrl + /</span> at any
            time.
          </div>
        </div>
      </div>
      ...
      >
        <kbd className="px-1 bg-primary-foreground/20 rounded text-[9px] font-mono">
          F1
        </kbd>

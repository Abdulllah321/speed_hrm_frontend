...
export default function GrossSalesReturnReport() {
    const { user } = useAuth();
    ...
    }, [selectedLocationId, defaultLocationId, dateRange, searchQuery, selectedCashierId, paymentModeGroup, minAmount, maxAmount, fbrOnly]);

    useEffect(() => {
        fetchReport();
    }, [selectedLocationId, defaultLocationId, dateRange, selectedCashierId, paymentModeGroup, fbrOnly, fetchReport]);
    ...
    const handleQueueXlsxExport = async () => {
        ...
        try {
            ...
            if (res && res.status && res.data?.jobId) {
                ...
                toast.success("Sales Return XLSX compilation queued!");
            } else {
    ...
    return (
        <div className="flex-1 space-y-6 p-8 pt-6">
            ...
            <div className="flex items-center justify-between space-y-2 no-print">
                <div>
                    <h2 className="text-3xl font-extrabold tracking-tight bg-gradient-to-r from-red-950 to-red-700 dark:from-white dark:to-red-400 bg-clip-text text-transparent">
                        Gross Sales Return Report
                    </h2>
                    <p className="text-sm text-muted-foreground mt-1">
                        Hierarchical analysis of returns, refunds, exchanges, and approved claims
                    </p>
                </div>
            ...
            <div className="hidden print:block mb-6 border-b pb-4">
                <h1 className="text-2xl font-bold text-center text-slate-900">Gross Sales Return Report</h1>
                                ...
                                {virtualItems.map((virtualRow) => {
                                    ...
                                    return (
                                        <tr key={virtualRow.index} className={cn("align-middle", styles.className)}>
                                            ...
                                            {/* Qty */}
                                            <td className="p-2 border-r text-right font-bold text-red-600 dark:text-red-400">
                                                {formatVal(row.qty)}
                                            </td>
                                            ...
                                            {/* Value Including Tax */}
                                            <td className="p-2 border-r text-right font-bold text-red-700 dark:text-red-450 bg-red-50/20 dark:bg-red-950/10">

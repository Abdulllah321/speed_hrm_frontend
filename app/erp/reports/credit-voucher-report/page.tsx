...
import {
  ...
  CreditVoucherReportData,
} from "@/lib/actions/credit-voucher-report";
...
export default function CreditVoucherReportPage() {
  const { user } = useAuth();
  ...
  const [reportData, setReportData] = useState<CreditVoucherReportData | null>(null);
  ...
  const fetchReport = useCallback(() => {
    ...
    startTransition(async () => {
      const result = await getCreditVoucherReport({
      ...
      if (result && result.status && result.data) {
        ...
        setReportData(null);
        toast.error("Failed to load Credit Voucher report data");
      }
    });
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      try {
        const res = await getCreditVoucherExportStatus(exportJobId);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      try {
        const res = await getCreditVoucherExportStatus(pdfJobId);
        if (res && res.status && res.data) {
          const { state, progress } = res.data;
  ...
  const handleExportExcelClick = async () => {
    ...
    if (exportState === "completed" && exportJobId) {
      ...
      const url = `${base}/api/pos-sales/reports/credit-voucher/export-download/${exportJobId}`;
    ...
    try {
      const res = await queueCreditVoucherExport({
        locationId: locationParam ?? "",
        startDate: dateRange.from.toISOString(),
        endDate: dateRange.to.toISOString(),
        format: "xlsx",
  ...
  const handleExportPdfClick = async () => {
    ...
    if (pdfExportState === "completed" && pdfJobId) {
      const base = getApiBaseUrl();
      const url = `${base}/api/pos-sales/reports/credit-voucher/export-download/${pdfJobId}`;
      window.open(url, "_blank");
  ...
  return (
    <div className="p-6 space-y-6 max-w-[1600px] mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-border pb-5 no-print">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1 flex items-center gap-1.5 font-medium">
            ...
            Store Credit Vouchers Issued & Settled Against Cash Memos for{" "}
      ...
      <div className="hidden print:block mb-6 border-b pb-4">
        ...
        <h2 className="text-lg font-bold text-center text-slate-700">Credit Voucher Sale Register Report</h2>
        ...
        <div className="flex flex-wrap items-end gap-4 flex-1">
          ...
          <div className="flex flex-col gap-1.5 flex-1 min-w-[260px]">
            ...
            <div className="relative">
              <Input
                placeholder="Search by Credit Voucher #, Base Cash Memo #, Customer Name, Phone..."
                value={searchQuery}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 no-print">
        <Card className="shadow-xs border-slate-100">
          <CardContent className="p-4 flex items-center justify-between">
            <div>
              <p className="text-[11px] font-bold text-muted-foreground uppercase tracking-wide">Credit Vouchers</p>
        ...
        <Card className="shadow-xs border-slate-100">
          <CardContent className="p-4 flex items-center justify-between">
            <div>
              <p className="text-[11px] font-bold text-muted-foreground uppercase tracking-wide">Total Credit Amount</p>
          ...
          <table className="w-full text-left border-collapse whitespace-nowrap min-w-[1900px]">
            <thead>
              <tr className="bg-[#0f172a] text-slate-100 border-b border-border/80 text-[10px] uppercase font-bold sticky top-0 z-10 shadow-sm">
                <th className="p-3.5 w-[180px] border-r bg-[#0f172a]">Credit Voucher #</th>
                <th className="p-3.5 w-[170px] border-r bg-[#0f172a]">Date Time</th>
                <th className="p-3.5 w-[240px] border-r bg-[#0f172a]">Customer Detail</th>
                <th className="p-3.5 w-[200px] border-r bg-[#0f172a]">Issued Outlet</th>
                <th className="p-3.5 w-[220px] border-r bg-[#0f172a]">Base Cash Memo / Source Inv #</th>
                ...
                <th className="p-3.5 w-[160px] border-r text-right bg-[#0f172a] font-extrabold text-emerald-300">Credit Amount (Rs.)</th>
                <th className="p-3.5 w-[220px] border-r bg-[#0f172a]">Settled In Cash Memo / Inv #</th>
            ...
            <tbody className="divide-y text-xs">
              {isPending ? (
                ...
                <tr>
                  <td colSpan={11} className="p-8 text-center text-muted-foreground font-medium">
                    No credit vouchers match the selected filters or search query.
                  </td>
                </tr>
              ) : (
                <>
                  ...
                  {virtualItems.map((virtualRow) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="p-3.5 border-r font-mono text-sky-700 dark:text-sky-300 flex items-center gap-1.5">
            ...
            {reportData && (
              <tfoot className="sticky bottom-0 z-10 shadow-md">
                <tr className="bg-[#0f172a] text-slate-100 font-extrabold border-t-2 border-slate-900 text-xs">
                  <td colSpan={6} className="p-3.5 border-r text-left uppercase tracking-wider font-black bg-[#0f172a]">
                    GRAND TOTALS ({filteredItems.length} CREDIT VOUCHERS)
                  </td>

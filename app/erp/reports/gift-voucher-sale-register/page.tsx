...
import {
  ...
  GiftVoucherSaleRegisterReportData,
} from "@/lib/actions/gift-voucher-sale-register";
...
export default function GiftVoucherSaleRegisterPage() {
  const { user } = useAuth();
  ...
  const [reportData, setReportData] = useState<GiftVoucherSaleRegisterReportData | null>(null);
  ...
  const fetchReport = useCallback(() => {
    ...
    startTransition(async () => {
      const result = await getGiftVoucherSaleRegisterReport({
      ...
      if (result && result.status && result.data) {
        ...
        setReportData(null);
        toast.error("Failed to load Gift Voucher Sale Register report data");
      }
    });
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      try {
        const res = await getGiftVoucherSaleRegisterExportStatus(exportJobId);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      try {
        const res = await getGiftVoucherSaleRegisterExportStatus(pdfJobId);
        if (res && res.status && res.data) {
          const { state, progress } = res.data;
  ...
  const handleExportExcelClick = async () => {
    ...
    if (exportState === "completed" && exportJobId) {
      ...
      const url = `${base}/api/pos-sales/reports/gift-voucher-sale-register/export-download/${exportJobId}`;
    ...
    try {
      const res = await queueGiftVoucherSaleRegisterExport({
        locationId: locationParam ?? "",
        startDate: dateRange.from.toISOString(),
        endDate: dateRange.to.toISOString(),
        format: "xlsx",
  ...
  const handleExportPdfClick = async () => {
    ...
    if (pdfExportState === "completed" && pdfJobId) {
      const base = getApiBaseUrl();
      const url = `${base}/api/pos-sales/reports/gift-voucher-sale-register/export-download/${pdfJobId}`;
      window.open(url, "_blank");
  ...
  const filteredItems = useMemo(() => {
    ...
    return reportData.items.filter((item) => {
      return (
        ...
        item.baseInvoiceNumber.toLowerCase().includes(query) ||
  ...
  return (
    <div className="p-6 space-y-6 max-w-[1600px] mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-border pb-5 no-print">
        <div>
          <h1 className="text-3xl font-extrabold tracking-tight flex items-center gap-2 text-slate-800 dark:text-slate-100">
            ...
            Gift Voucher Sale Register Report
          </h1>
          <p className="text-sm text-muted-foreground mt-1 flex items-center gap-1.5 font-medium">
            ...
            Gift Vouchers Issuance, Redemption & Settlement Register for{" "}
      ...
      <div className="hidden print:block mb-6 border-b pb-4">
        ...
        <h2 className="text-lg font-bold text-center text-slate-700">Gift Voucher Sale Register Report</h2>
        ...
        <div className="flex flex-wrap items-end gap-4 flex-1">
          ...
          <div className="flex flex-col gap-1.5 flex-1 min-w-[260px]">
            ...
            <div className="relative">
              <Input
                placeholder="Search by Voucher #, Type, Outlet, Customer Name, Base Invoice #..."
                value={searchQuery}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 no-print">
        <Card className="shadow-xs border-slate-100">
          <CardContent className="p-4 flex items-center justify-between">
            <div>
              <p className="text-[11px] font-bold text-muted-foreground uppercase tracking-wide">Vouchers Issued</p>
      ...
      <div className="border rounded-xl shadow-sm bg-background overflow-x-auto no-print">
        <div ref={parentRef} className="overflow-auto max-h-[700px] min-w-[1800px]">
          <table className="w-full text-left border-collapse whitespace-nowrap min-w-[1800px]">
            <thead>
              <tr className="bg-[#0f172a] text-slate-100 border-b border-border/80 text-[10px] uppercase font-bold sticky top-0 z-10 shadow-sm">
                <th className="p-3.5 w-[180px] border-r bg-[#0f172a]">Gift Voucher #</th>
                <th className="p-3.5 w-[120px] border-r bg-[#0f172a]">Type</th>
                <th className="p-3.5 w-[170px] border-r bg-[#0f172a]">Date Time</th>
                <th className="p-3.5 w-[200px] border-r bg-[#0f172a]">Issued Outlet</th>
                <th className="p-3.5 w-[220px] border-r bg-[#0f172a]">Customer Detail</th>
                <th className="p-3.5 w-[130px] border-r bg-[#0f172a]">Valid Till</th>
                <th className="p-3.5 w-[180px] border-r bg-[#0f172a]">Base Inv / Claim #</th>
                ...
                <th className="p-3.5 w-[180px] border-r bg-[#0f172a]">Settled In Invoice</th>
                <th className="p-3.5 w-[170px] bg-[#0f172a]">Settled Date Time</th>
              </tr>
            </thead>
            <tbody className="divide-y text-xs">
              {isPending ? (
                <tr>
                  <td colSpan={11} className="p-8 text-center text-muted-foreground font-medium">
                    <div className="flex items-center justify-center gap-2">
                      ...
                      Loading Gift Voucher Sale Register...
                    </div>
                  </td>
                </tr>
                ...
                <tr>
                  <td colSpan={11} className="p-8 text-center text-muted-foreground font-medium">
                    No gift voucher sales match the selected filters or search query.
                  </td>
                </tr>
              ) : (
                <>
                  ...
                  {virtualItems.map((virtualRow) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {/* Type */}
                        <td className="p-3.5 border-r font-bold text-slate-700 dark:text-slate-300 uppercase text-[10px]">
                          {item.voucherType}
                        </td>
                        ...
                        {/* Settled Date Time */}
                        <td className="p-3.5 text-slate-600 dark:text-slate-400">{item.settledDateTime}</td>
                      </tr>
                    );
                  })}
                  {paddingBottom > 0 && (
            ...
            {reportData && (
              <tfoot className="sticky bottom-0 z-10 shadow-md">
                <tr className="bg-[#0f172a] text-slate-100 font-extrabold border-t-2 border-slate-900 text-xs">
                  ...
                  <td colSpan={2} className="p-3.5 bg-[#0f172a] text-sky-300 font-mono text-xs">

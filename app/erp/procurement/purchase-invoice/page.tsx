...
export default function PurchaseInvoiceListPage() {
  ...
  useEffect(() => {
    fetchInvoices();
  }, [page, limit, searchTerm, statusFilter, paymentStatusFilter]);
  ...
  const fetchInvoices = async () => {
    try {
      ...
      if (response?.pagination) {
        setTotal(response.pagination.total ?? (Array.isArray(invoiceData) ? invoiceData.length : 0));
        setTotalPages(response.pagination.totalPages ?? 1);
      } else {
        const totalCount = Array.isArray(invoiceData) ? invoiceData.length : 0;
        setTotal(totalCount);
        setTotalPages(1);
      }
    } catch (error) {

...
function getGroupedItems(items: any[]) {
  ...
  items.forEach((item) => {
    ...
    const taxAmt = Number(
      item.taxAmount || ((qty * unitCost - discAmt) * taxRate) / 100,
    );
...
interface PurchaseInvoice extends Omit<
  ApiPurchaseInvoice,
  | "subtotal"
  | "advanceTaxRate"
  | "advanceTaxAmount"
...
export default function PurchaseInvoiceDetailPage() {
      ...
      const totalsRow = [
        ...
        items.reduce((s, i) => {
          const qty = Number(i.quantity || 0),
            up = Number(i.unitPrice || 0),
            dr = Number(i.discountRate || 0);
        ...
        items.reduce((s, i) => {
          ...
          const tr = Number(i.taxRate || 0),
            ta = Number(i.taxAmount || ((qty * up - da) * tr) / 100);
    ...
    try {
      ...
      rows.push([
        ...
        items.reduce((s, i) => {
          ...
          const tr = Number(i.taxRate || 0),
            ta = Number(i.taxAmount || ((q * u - da) * tr) / 100);
          return s + (q * u - da + ta);
        }, 0),
        "",
        "",
      ...
      link.setAttribute(
        "download",
        `purchase-invoice-${invoice.invoiceNumber}.csv`,
      );
  ...
  const handleApprove = async () => {
    try {
      ...
      const errorMessage =
        error.message || error.error?.message || "Failed to approve invoice";
  ...
  const uniqueTaxRates = Array.from(
    new Set((invoice?.items || []).map((i: any) => Number(i.taxRate || 0))),
  );
  const taxRateStr =
    uniqueTaxRates.length > 0
      ? uniqueTaxRates.map((r) => `${r}%`).join(", ")
      : "18%";
  ...
  return (
    <PermissionGuard permissions="erp.procurement.pi.read">
      <>
        ...
        `}</style>
        <div
          className="container mx-auto p-6 print:hidden"
          key={invoice.status}
        >
            ...
            <div className="flex gap-2" key={`buttons-${invoice.status}`}>
              ...
              {invoice.status === "DRAFT" && (
                <>
                  ...
                  <PermissionGuard
                    permissions="erp.procurement.pi.post"
                    fallback={null}
                  >
                    <Button
                      variant="default"
              ...
              {invoice.status === "APPROVED" && (
                ...
                >
                  <Button
                    onClick={() =>
                      router.push(
                        `/erp/finance/payment-voucher/create?invoiceId=${invoice.id}`,
                      )
                    }
                  >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              {invoice.paymentVouchers &&
                invoice.paymentVouchers.length > 0 && (
                  <Card>
                    <CardHeader>

          ...
          <Card className="bg-gradient-to-r from-slate-900/90 to-slate-950/95 text-white border-slate-800 shadow-xl overflow-hidden relative backdrop-blur-md">
            ...
            <CardContent className="relative py-4">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 relative">
                ...
                {/* Step 2: Checked */}
                <div className="flex items-start md:flex-col gap-4 md:text-center w-full md:w-1/3 z-10">
                  <div className={`flex items-center justify-center w-12 h-12 rounded-full border-2 md:mx-auto transition-all duration-300 ${order.status === 'PENDING_CHECKER'
                ...
                {/* Step 3: Authorized */}
                <div className="flex items-start md:flex-col gap-4 md:text-center w-full md:w-1/3 z-10">
                  <div className={`flex items-center justify-center w-12 h-12 rounded-full border-2 md:mx-auto transition-all duration-300 ${order.status === 'OPEN' || order.status === 'CLOSED' || order.status === 'PARTIALLY_RECEIVED' || order.status === 'RECEIVED'
                      ? 'bg-emerald-500 border-emerald-400 text-white shadow-lg shadow-emerald-500/20'
          ...
          <div className="flex gap-2">
            ...
            {(order.status === "PENDING_CHECKER" ||
              order.status === "PENDING_AUTHORIZER") && (
                <Button variant="outline" asChild>
                  <Link
                    href={`/erp/procurement/purchase-order/${order.id}/edit`}
        ...
        <Card className="bg-gradient-to-r from-slate-900/90 to-slate-950/95 text-white border-slate-800 shadow-xl overflow-hidden relative backdrop-blur-md">
          ...
          <CardContent className="relative py-4">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 relative">
              ...
              <div className="flex items-start md:flex-col gap-4 md:text-center w-full md:w-1/3 z-10">
                <div
                  className={`flex items-center justify-center w-12 h-12 rounded-full border-2 md:mx-auto transition-all duration-300 ${order.status === "PENDING_CHECKER"
                      ? "bg-amber-500 border-amber-400 text-white animate-pulse shadow-lg shadow-amber-500/20"
                      : order.status === "REJECTED" && !order.checkedById
                        ? "bg-rose-500 border-rose-400 text-white shadow-lg shadow-rose-500/20"
                        : order.checkedById
                ...
                <div className="flex flex-col md:items-center">
                  ...
                  {order.checkedById ? (
                    <>
                      ...
                      <span className="text-[10px] text-slate-500 mt-0.5">
                        {order.checkedAt
                          ? new Date(order.checkedAt).toLocaleDateString()
                          : ""}
                      </span>
                    </>
                  ) : order.status === "PENDING_CHECKER" ? (
                    <span className="text-xs text-amber-400 font-medium animate-pulse mt-0.5">
              ...
              <div className="flex items-start md:flex-col gap-4 md:text-center w-full md:w-1/3 z-10">
                <div
                  className={`flex items-center justify-center w-12 h-12 rounded-full border-2 md:mx-auto transition-all duration-300 ${order.status === "OPEN" ||
                      order.status === "CLOSED" ||
                      order.status === "PARTIALLY_RECEIVED" ||
                ...
                <div className="flex flex-col md:items-center">
                  ...
                  {order.authorizedById ? (
                    <>
                      ...
                      <span className="text-[10px] text-slate-500 mt-0.5">
                        {order.authorizedAt
                          ? new Date(order.authorizedAt).toLocaleDateString()
                          : ""}
                      </span>
                    </>
                  ) : order.status === "PENDING_AUTHORIZER" ? (
                    <span className="text-xs text-blue-400 font-medium animate-pulse mt-0.5">
        ...
        {((order.status === "PENDING_CHECKER" && canCheck) ||
          (order.status === "PENDING_AUTHORIZER" && canAuthorize)) && (
            <Card className="bg-gradient-to-r from-blue-50/50 to-indigo-50/50 dark:from-slate-900/40 dark:to-slate-900/20 border-blue-200/60 dark:border-slate-800 shadow-md">
              ...
              <CardContent className="space-y-4">
                <p className="text-sm text-blue-700 dark:text-slate-300">
                  This Purchase Order is currently in{" "}
                  <strong>
                    {order.status === "PENDING_CHECKER"
                  ...
                  order to forward it to the next step, or reject it.
                </p>
                <div className="flex gap-4">
                  <Button
                    className="bg-emerald-600 hover:bg-emerald-700 text-white font-medium shadow-md hover:shadow-lg transition-all"
                  ...
                  switch (order.status) {
                    case "PENDING_CHECKER":
                      return (
                        <Badge
                          variant="outline"
                          className="bg-amber-50 text-amber-700 border-amber-200 font-medium dark:bg-amber-950/30 dark:text-amber-400 dark:border-amber-900"
                        >
                          Pending Checker
                        </Badge>
                      );
                    case "PENDING_AUTHORIZER":
                      return (
                        <Badge
                          variant="outline"
                          className="bg-blue-50 text-blue-700 border-blue-200 font-medium dark:bg-blue-950/30 dark:text-blue-400 dark:border-blue-900"
                        >
                          Pending Authorizer
                        </Badge>
                      );
                    case "OPEN":
                      return (
                        <Badge
                          variant="outline"
                          className="bg-emerald-50 text-emerald-700 border-emerald-200 font-medium dark:bg-emerald-950/30 dark:text-emerald-400 dark:border-emerald-900"
                        >
                          Open
                        </Badge>
                      );
                    case "REJECTED":
                      return (
                        <Badge
                          variant="outline"
                          className="bg-rose-50 text-rose-700 border-rose-200 font-medium dark:bg-rose-950/30 dark:text-rose-400 dark:border-rose-900"
                        >
                          Rejected
                        </Badge>
                      );
                    case "CLOSED":
                      return (
                        <Badge
                          variant="outline"
                          className="bg-slate-50 text-slate-700 border-slate-200 font-medium dark:bg-slate-950/30 dark:text-slate-400 dark:border-slate-900"
                        >
                          Closed
                        </Badge>
                      );
                    case "PARTIALLY_RECEIVED":
                      return (
                        <Badge
                          variant="outline"
                          className="bg-orange-50 text-orange-700 border-orange-200 font-medium dark:bg-orange-950/30 dark:text-orange-400 dark:border-orange-900"
                        >
                          Partially Received
                        </Badge>
                      );
                    case "RECEIVED":
                      return (
                        <Badge
                          variant="outline"
                          className="bg-indigo-50 text-indigo-700 border-indigo-200 font-medium dark:bg-indigo-950/30 dark:text-indigo-400 dark:border-indigo-900"
                        >
                          Received
                        </Badge>
                      );
                ...
                <div className="w-[55%] pt-4">
                  <div className="flex gap-2 font-bold text-xs sm:text-[13px]">
                    ...
                    <span className="underline decoration-1 underline-offset-2 break-words">
                      {numberToWords(Number(order.totalAmount || 0))}
                    </span>
                  </div>
                </div>
                <div className="w-[25%] pr-2 text-right">
                  <div className="text-xs sm:text-[13px] text-gray-700">
                    Subtotal:
                  </div>
                  <div className="text-xs sm:text-[13px] text-gray-700">
                    Tax:
                  ...
                  <div className="font-bold text-xs sm:text-[13px] mt-1">
                    Total:
                  </div>
                </div>
                <div className="w-[20%] text-right">
                  <div className="tabular-nums text-xs sm:text-[13px] text-gray-700">
                    {fmt(Number(order.subtotal || 0))}
                  </div>
                  <div className="tabular-nums text-xs sm:text-[13px] text-gray-700">
                    {fmt(Number(order.taxAmount || 0))}
                  </div>
                  <div className="tabular-nums text-xs sm:text-[13px] text-gray-700">
                    -{fmt(Number(order.discountAmount || 0))}
                  </div>
                  <div
                    className="ml-auto border-t border-black pb-0.5 mt-1"
                    style={{ borderBottom: "3px double black" }}
                  >
                    <span className="tabular-nums font-bold text-xs sm:text-[13px] block pt-0.5">

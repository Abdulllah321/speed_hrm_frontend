...
export default function PurchaseOrderDetail({
  ...
  const handleExport = async () => {
    ...
    try {
      setIsExporting(true);
      await queuePurchaseOrderExport({ poId: order.id });
      toast.success("Purchase Order export queued. You will receive a notification when the Excel file is ready to download.");
    } catch (error: any) {
  ...
  if (!order)
    return <div className="p-6 text-center text-muted-foreground">Purchase order not found.</div>;

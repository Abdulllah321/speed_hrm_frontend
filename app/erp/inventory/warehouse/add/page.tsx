    ...
    const columns: ColumnDef<Warehouse>[] = useMemo(() => [
        {
            accessorKey: 'code',
            header: 'Code',
            cell: ({ row }) => <span className="font-mono font-bold text-sm text-primary">{row.original.code}</span>
        },
        {
            accessorKey: 'name',
            header: 'Name',
        ...
        },
        {
            accessorKey: 'type',
            header: 'Type',
            cell: ({ row }) => <Badge variant="outline" className="bg-background">{row.original.type}</Badge>
        },
        {
            accessorKey: 'address',
            header: 'Address',
            cell: ({ row }) => <span className="max-w-xs truncate text-muted-foreground text-sm">{row.original.address || '-'}</span>
        },
        {
    ...
    return (
        <div className="p-6 space-y-6">
            ...
            <Sheet open={showForm} onOpenChange={setShowForm}>
                <SheetContent side="bottom" className="sm:max-w-full lg:max-w-3xl mx-auto rounded-t-xl overflow-y-auto max-h-[90vh]">
            ...
            <div className="grid grid-cols-1 gap-6">
                <DataTable
                    ...
                    onRowEdit={(w) => router.push(`/erp/inventory/warehouse/edit/${w.id}`)}

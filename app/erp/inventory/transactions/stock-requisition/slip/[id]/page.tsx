...
export default function StockRequisitionSlipPage() {
  ...
  const loadRequisitionDetails = async () => {
    try {
      setLoading(true);
      const res = await stockRequisitionApi.getById(id);
      if (res && res.status) {
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen space-y-4">
        ...
        <p className="text-muted-foreground text-sm font-semibold">Loading Requisition Slip...</p>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gray-50 print:bg-white text-black py-6">
      ...
      <div className="print:hidden bg-white border p-4 flex justify-between items-center shadow-sm max-w-4xl mx-auto rounded-md mb-6">
        ...
        <Button className="bg-indigo-600 hover:bg-indigo-700 font-bold" onClick={printSlip}>
          <Printer className="h-4 w-4 mr-2" /> Print SRN Slip
        </Button>
      </div>

    ...
    <div className="w-full bg-white text-black p-8 font-sans print:p-0 box-border text-[11px]">
      ...
      <div className="flex justify-between items-start mb-8 pb-4 border-b border-black">
        ...
        <div className="w-[25%] flex flex-col justify-center">
          ...
          />
          <h2 className="text-sm font-bold tracking-tight mt-1">
            Speed (pvt.) Limited
          </h2>
      ...
      <table className="w-full border-collapse border border-gray-400">
        <thead>
          <tr className="bg-gray-100 border-b border-gray-400">
            <th
              rowSpan={2}
              className="border border-gray-400 text-center py-2 font-bold uppercase w-[5%]"
            >
              Sr.
            </th>
            <th
              rowSpan={2}
              className="border border-gray-400 text-center py-2 font-bold uppercase w-[12%]"
            >
              Code
            </th>
            <th
              rowSpan={2}
              className="border border-gray-400 text-left px-3 py-2 font-bold uppercase w-[33%]"
            >
              Account Description
            </th>
            {showOpening && (
              <th
                colSpan={2}
                className="border border-gray-400 text-center py-1 font-bold uppercase"
              >
                Opening Balance
              </th>
            )}
          ...
          <tr className="bg-gray-50 border-b border-gray-400">
            {showOpening && (
              <>
                <th className="border border-gray-400 text-right px-2 py-1.5 font-semibold text-[9px] uppercase w-[8%]">
                  DR
                </th>
                <th className="border border-gray-400 text-right px-2 py-1.5 font-semibold text-[9px] uppercase w-[8%]">
                  CR
        ...
        <tbody>
          {rows.length === 0 ? (
            <tr>
              <td
                colSpan={totalCols}
                className="text-center py-8 text-gray-500 font-semibold italic border border-gray-400"
              >
            ...
            rows.map((row, i) => {
              ...
              return (
                ...
                >
                  <td className="border border-gray-300 text-center py-1.5 font-mono text-[10px]">
                    {i + 1}
                  </td>
                  <td className="border border-gray-300 text-center py-1.5 font-mono text-[10px]">
                    {row.code}
                  ...
                  {showTransactions && (
                    <>
                      <td className="border border-gray-300 text-right px-2 py-1.5 font-mono text-[10px] tabular-nums">
                        {row.transactionDebit > 0
                          ? fmt(row.transactionDebit)
                          : ""}
                      </td>
                      <td className="border border-gray-300 text-right px-2 py-1.5 font-mono text-[10px] tabular-nums">
                        {row.transactionCredit > 0
                          ? fmt(row.transactionCredit)
                          : ""}
                      </td>
                    </>
                  )}
        ...
        <tfoot>
          <tr className="bg-gray-100 font-bold border-t-2 border-black text-[11px]">
            <td
              colSpan={3}
              className="border border-gray-400 text-right px-3 py-2 uppercase tracking-wide"
            >
              Grand Total
            </td>
            {showOpening && (
              <>
                <td className="border border-gray-400 text-right px-2 py-2 font-mono tabular-nums">
      ...
      {!data.balanced && (
        <div className="mt-4 p-3 bg-red-50 border border-red-300 rounded text-red-800 font-bold flex justify-between">
          <span>
            ⚠ WARNING: Trial Balance is NOT balanced. Please review transactions
            for ledger discrepancies.
          </span>
          <span>
            Difference:{" "}
            {fmt(Math.abs((data.totalDebit ?? 0) - (data.totalCredit ?? 0)))}
          </span>
        </div>
      )}

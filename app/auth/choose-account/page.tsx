    ...
    async function loadProfiles() {
      ...
      try {
        ...
        try {
          ...
          console.error(
            "Failed to parse rememberedProfiles from localStorage",
            e,
          );
          // Clear invalid data
        ...
        remembered.forEach((p: any) =>
          mergedMap.set(p.email, { ...p, isActive: false }),
        );

        // Overwrite with active sessions
        if (Array.isArray(activeProfiles)) {
          activeProfiles.forEach((p: any) =>
            mergedMap.set(p.email, { ...p, isActive: true }),
          );
        }
  ...
  return (
    <Card className="shadow-2xl backdrop-blur-sm bg-card/95">
      <CardHeader className="text-center space-y-2 pb-6">
        ...
        <CardDescription>
          Select an account to continue to Speed (pvt.) Limited
        </CardDescription>
      </CardHeader>
